...
func TestDiscoverEmbeddedMigrationsUsesCompactedBaseline(t *testing.T) {
...
func TestEmbeddedBaselineSetsUTF8MB4Defaults(t *testing.T) {
	content, err := fs.ReadFile(dbmigrations.Files, "V1__schema.sql")
	if err != nil {
		t.Fatalf("read V1 baseline failed: %v", err)
	}
	...
	for _, expected := range []string{
		...
		if !strings.Contains(text, expected) {
			t.Fatalf("expected V1 baseline to contain %q", expected)
		}
	}

	statements, err := splitStatements(text)
	if err != nil {
		t.Fatalf("split V1 baseline failed: %v", err)
	}
...
func TestEmbeddedBaselineDeclaresPipelineRunDeadlineColumns(t *testing.T) {
	content, err := fs.ReadFile(dbmigrations.Files, "V1__schema.sql")
	if err != nil {
		t.Fatalf("read V1 baseline failed: %v", err)
	}
...
func TestEmbeddedBaselineDeclaresMCPCallAuditTable(t *testing.T) {
	content, err := fs.ReadFile(dbmigrations.Files, "V1__schema.sql")
	if err != nil {
		t.Fatalf("read V1 baseline failed: %v", err)
	}
...
func TestEmbeddedSchemaIncludesTemplateParametersExtraTipColumn(t *testing.T) {
	...
	if !strings.Contains(text, "`extra_tip` text DEFAULT NULL") {
...
func TestEmbeddedBaselineDeclaresAgentAndSystemSettingColumns(t *testing.T) {
	content, err := fs.ReadFile(dbmigrations.Files, "V1__schema.sql")
	if err != nil {
		t.Fatalf("read V1 baseline failed: %v", err)
	}
	text := string(content)
	for _, expected := range []string{"CREATE TABLE `agents`", "task_concurrency", "dockerhub_mirrors", "dockerhub_mirrors_configured", "CREATE TABLE `system_settings`", "`key` varchar(128) NOT NULL", "UNIQUE KEY `idx_system_settings_key` (`key`)"} {
		if !strings.Contains(text, expected) {
...
func TestEmbeddedBaselineIncludesAIFoundation(t *testing.T) {
	content, err := fs.ReadFile(dbmigrations.Files, "V1__schema.sql")
	if err != nil {
		t.Fatalf("read V1 schema failed: %v", err)
	}
...
func TestEmbeddedSeedDataUsesAIFoundationTemplateType(t *testing.T) {
	content, err := fs.ReadFile(dbmigrations.Files, "V2__bootstrap_seed_data.sql")
	if err != nil {
		t.Fatalf("read V2 seed data failed: %v", err)
	}
	text := string(content)
	for _, expected := range []string{"`template_type`", "'ai-service','ai','vm'", "'ai-service','ai','k8s'"} {
		if !strings.Contains(text, expected) {
			t.Fatalf("expected V2 seed data to contain %q, got %s", expected, text)
		}
	}
	if strings.Contains(text, "'llm'") {
		t.Fatalf("expected V2 seed data not to contain legacy llm template_type")
	}
}

func TestEmbeddedBaselineDeclaresAgentRuntimeCapabilityAlignment(t *testing.T) {
	content, err := fs.ReadFile(dbmigrations.Files, "V1__schema.sql")
	if err != nil {
		t.Fatalf("read V1 baseline failed: %v", err)
	}
