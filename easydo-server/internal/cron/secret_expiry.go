...
func checkExpiringSecrets(db *gorm.DB, webhookService *services.WebhookService) {
	...
	for _, secret := range secrets {
		if secret.ExpiresAt != nil {
			...
			webhookService.SendWebhook(secret.WorkspaceID, "secret.expiring", map[string]interface{}{
				"secret_id":   secret.ID,
	...
	for _, secret := range autoRotateSecrets {
		webhookService.SendWebhook(secret.WorkspaceID, "secret.auto_rotate", map[string]interface{}{
			"secret_id":   secret.ID,
			"secret_name": secret.Name,
			"message":     "密钥将在3天内自动轮换",
		})
