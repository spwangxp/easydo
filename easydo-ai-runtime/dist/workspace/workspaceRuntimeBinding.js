...
export class WorkspaceRuntimeBinding {
    ...
    executionClient;
    options;
    constructor(store, service, executionClient, options) {
        this.store = store;
    ...
    async acquire(run) {
        ...
        if (!session)
            throw new RuntimeDomainError('session_not_found', 'Session not found', 404);
        ...
        const claimed = await this.store.claimAgentWorkspaceLease(run.workspace_id, workspaceRuntimeID, instanceID, this.leaseExpiry());
        if (!claimed) {
        ...
        return {
            ...
            renew: async () => Boolean(await this.store.renewAgentWorkspaceLease(run.workspace_id, workspaceRuntimeID, instanceID, leaseEpoch, this.leaseExpiry())),
            release: async () => {
                if (released)
                    return;
                released = true;
