...
describe('AgentWorkspaceService', () => {
    ...
    it('rejects recycle while an Agent run owns a live workspace lease', async () => {
        ...
        const workspace = await service.ensureDefault(actor());
        await store.claimAgentWorkspaceLease(workspace.workspace_id, workspace.workspace_runtime_id, 'runtime-active', '2026-07-12T00:01:00.000Z', '2026-07-12T00:00:00.000Z');
    ...
    it.each(['ready', 'paused'])('serializes concurrent recycle requests from %s', async (status) => {
        const store = createMemoryRuntimeStore();
        let releaseRecycle;
        const recycleGate = new Promise((resolve) => { releaseRecycle = resolve; });
        let recycleCalls = 0;
        const service = new AgentWorkspaceService(store, {
            async provision(workspace) { return { sandbox_id: `sandbox-${workspace.workspace_runtime_id}` }; },
            async connect() { },
        ...
        const workspace = await service.ensureDefault(actor());
        if (status === 'paused')
            await service.pause(actor(), workspace.workspace_runtime_id);
