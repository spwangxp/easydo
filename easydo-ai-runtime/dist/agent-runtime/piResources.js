...
import { filterToolsByMode, normalizeSubagentMode, operationTypeFromToolMeta } from '../services/subagentMode.js';
export class PiToolApprovalRequiredError extends Error {
    approvalID;
    callID;
    toolName;
    reason;
    constructor(approvalID, callID, toolName, reason) {
        super(reason);
...
export function buildPiHarnessResources(input) {
...
function skillLoaderTool(resources, input) {
    return {
        ...
        async execute(toolCallId, params) {
            ...
            try {
                ...
                if (!loaded.content.trim())
                    throw new Error(`Skill ${resource.name} has no readable instructions`);
...
function skillInstructionContent(resource) {
    const spec = asRecord(resource.spec);
    return firstString(spec.instructions, spec.markdown, spec.content, spec.prompt);
}
function selectedSkillResources(profile, resources) {
...
function selectedMcpResources(profile, resources) {
...
function skillFromResource(resource, includeContent) {
    const spec = asRecord(resource.spec);
    return {
...
function mcpToolsFromResource(resource, ref, input) {
    ...
    return definitions.map((tool) => agentToolFromMcpDefinition(resource, ref, tool, input)).filter((tool) => Boolean(tool));
}
function agentToolFromMcpDefinition(resource, ref, value, input) {
    ...
    return {
        ...
        async execute(toolCallId, params) {
            if (!input.executeTool)
                throw new Error(`Pi MCP tool adapter is not wired yet: ${name}`);
...
function effectiveMcpToolPolicy(profile, resource, ref) {
...
function scopedMcpPermissionRule(rule, resource, ref, scope) {
    const serverKeys = [
...
function toolPermissionRules(policy, scope) {
    const toolMap = asRecord(policy.tools);
    const inlineMap = Object.fromEntries(Object.entries(policy).filter(([key]) => !['tools', 'rules', 'default_decision', 'defaultDecision', 'default_matched_rule', 'defaultMatchedRule', 'default_scope', 'defaultScope'].includes(key)));
    ...
    const rules = entries.flatMap(([toolName, value]) => {
        ...
        if (permissionKey)
            rule.permission_key = permissionKey;
        return [rule];
    });
    if (Array.isArray(policy.rules))
        rules.push(...policy.rules.map(asRecord));
    return rules;
}
...
function jsonSchemaToPiParameters(value) {
...
function normalizeJSONSchema(schema) {
    ...
    if (firstString(schema.description))
        normalized.description = firstString(schema.description);
    ...
    if (Object.keys(properties).length > 0) {
        ...
        normalized.properties = Object.fromEntries(Object.entries(properties).map(([key, property]) => [key, normalizeJSONSchema(asRecord(property))]));
    }
    const items = asRecord(schema.items);
    if (Object.keys(items).length > 0)
        normalized.items = normalizeJSONSchema(items);
    return Object.keys(normalized).length > 0 ? normalized : Type.Any();
}
function subagentSpawnTool(subagents, input) {
    return {
        name: 'subagent.spawn',
        ...
        async execute(toolCallId, params) {
            ...
            if (!input.executeSubagent)
                throw new Error('Pi subagent adapter is not wired yet: subagent.spawn');
...
function refMatchesResource(ref, resource) {
    ...
    return Boolean(expected) && (expected === normalizeResourceID(resource.resource_key) ||
...
function skillIsLoaded(resource, loadedSkillIDs) {
