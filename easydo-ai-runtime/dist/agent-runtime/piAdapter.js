export class PiEventAdapter {
        ...
        switch (event.type) {
            case 'message_update': {
                ...
                if (ame && typeof ame === 'object' && 'type' in ame) {
                    return this.handleAssistantMessageEvent(ame);
                }
     ...
     */
    handleAssistantMessageEvent(ame) {
        const type = ame.type;
        const contentIndex = typeof ame.contentIndex === 'number' ? ame.contentIndex : 0;
        switch (type) {
            case 'text_start':
                return [this.textEvent('session.text.started', contentIndex, { text_id: this.textID(contentIndex) })];
            case 'text_delta': {
                const delta = String(ame.delta ?? '');
                if (!delta)
                    return [];
                this.emittedText = true;
                return [this.textEvent('session.text.delta', contentIndex, { text_id: this.textID(contentIndex), delta })];
            }
            case 'text_end':
                // HarnessRunner owns session.text.ended (it accumulates deltas and emits the final text)
                return [];
            case 'thinking_start':
                return [this.reasoningEvent('session.reasoning.started', contentIndex, {})];
            case 'thinking_delta': {
                const delta = String(ame.delta ?? '');
                if (!delta)
                    return [];
                return [this.reasoningEvent('session.reasoning.delta', contentIndex, { delta })];
            }
            case 'thinking_end':
                return [this.reasoningEvent('session.reasoning.ended', contentIndex, { text: String(ame.content ?? '') })];
            case 'toolcall_start': {
                const info = extractToolCallInfo(ame.partial, contentIndex);
                return [this.toolInputEvent('session.tool.input.started', info.id, info.name, {})];
            }
            case 'toolcall_delta': {
                const delta = String(ame.delta ?? '');
                if (!delta)
                    return [];
                const info = extractToolCallInfo(ame.partial, contentIndex);
                return [this.toolInputEvent('session.tool.input.delta', info.id, info.name, { delta })];
            }
            case 'toolcall_end': {
                const toolCall = asRecord(ame.toolCall);
                const callId = typeof toolCall.id === 'string' ? toolCall.id : `call:${contentIndex}`;
                const args = asRecord(toolCall.arguments);
                return [this.toolInputEvent('session.tool.input.ended', callId, undefined, { text: JSON.stringify(args) })];
            }
            default:
                return [];
        }
    ...
    reasoningID(contentIndex) {
        return `${this.assistantMessageID}:reasoning:${contentIndex}`;
    }
    textEvent(type, contentIndex, extra) {
        return {
            ...this.base(type),
            assistant_message_id: this.assistantMessageID,
            ...extra
    ...
    reasoningEvent(type, contentIndex, extra) {
        return {
            ...this.base(type),
            assistant_message_id: this.assistantMessageID,
    ...
    toolInputEvent(type, callId, toolName, extra) {
        const payload = {
            ...this.base(type),
            assistant_message_id: this.assistantMessageID,
            call_id: callId,
...
function isToolUpdateEvent(event) {
    const record = asRecord(event);
    return typeof record.toolCallId === 'string' && typeof record.toolName === 'string';
}
function isToolEndEvent(event) {
    const record = asRecord(event);
    return typeof record.toolCallId === 'string' && typeof record.toolName === 'string';
}
...
function readContent(value) {
    return Array.isArray(value)
        ? value.filter((item) => Boolean(item && typeof item === 'object' && 'type' in item))
        : [];
}
function readOutputPaths(result) {
 ...
 */
function extractToolCallInfo(partial, contentIndex) {
    const record = asRecord(partial);
    const content = record.content;
    if (!Array.isArray(content))
        return { id: `call:${contentIndex}`, name: 'unknown' };
