...
import { createAgentResourceVersion, stableJSONStringify } from '../domain/runtime.js';
...
describe('MariadbRuntimeStore entries', () => {
    it('preserves numeric database IDs and numeric string keys when reading Profile refs', async () => {
        ...
        const store = new MariadbRuntimeStore(pool);
        const readRefs = store.profileRefs.bind(store);
    ...
    it('guards resource deletion while a Run is awaiting a decision', async () => {
        const pool = new FakePool();
        const store = new MariadbRuntimeStore(pool);
        pool.queryResults.push([{ id: 7, workspace_id: 11, resource_kind: 'skill', resource_key: 'guarded-skill' }], [], [], [{ profile_snapshot_json: JSON.stringify({ frozen_resources: { skills: [{ id: 7 }] } }) }]);
    ...
    it('uses one conditional update to claim a workspace recycle lifecycle', async () => {
        ...
        const store = new MariadbRuntimeStore(pool);
        await expect(store.beginAgentWorkspaceRecycle(1, 'aws_w1_u7_default', 7, 'recycle-operation-a', '2026-07-12T00:05:00.000Z', '2026-07-12T00:00:00.000Z')).resolves.toBeUndefined();
        const statement = pool.statements.at(-1);
    ...
    it('serializes queue enqueue under the Session lock before allocating sequence and position', async () => {
        const pool = new FakePool();
        pool.queryResults.push([{ id: 9 }], [], [{ active_count: 2, max_item_seq: 4, max_position: 2 }]);
        const store = new MariadbRuntimeStore(pool);
    ...
    it('rolls back a queue enqueue when its lifecycle event insert fails', async () => {
        ...
        pool.queryResults.push([{ id: 9 }], [], [{ active_count: 0, max_item_seq: 0, max_position: 0 }], [{ last_seq: 4 }]);
        const store = new MariadbRuntimeStore(pool);
        await expect(store.enqueueSessionQueueItem({
            workspace_id: 11,
            session_id: 9,
    ...
    it('commits cancel and claim lifecycle events with their queue transitions', async () => {
        ...
        const cancelStore = new MariadbRuntimeStore(cancelPool);
        ...
        claimPool.queryResults.push([{ id: 9 }], [queueRow({ status: 'pending', claimed_by: null, claim_epoch: 0 })]);
        const claimStore = new MariadbRuntimeStore(claimPool);
    ...
    it('reorders only when the request contains the exact pending queue set', async () => {
        const row = (queueItemID, itemSeq, position) => ({
    ...
    it('atomically consumes a claimed queue item with Pi entries, run, session progress, and started event', async () => {
        const pool = new FakePool();
        pool.queryResults.push([{ id: 9 }], [queueRow({ mode: 'follow_up', target_run_id: null })], [], [{ last_seq: 8 }]);
        const store = new MariadbRuntimeStore(pool);
    ...
    it('returns already_consumed without creating another queued Pi run', async () => {
        const pool = new FakePool();
        pool.queryResults.push([{ id: 9 }], [queueRow({
                mode: 'follow_up',
                status: 'consumed',
                consumed_runtime_run_id: 'r_wb_000009_000001'
            })], [runRow()]);
    ...
    it('returns stale_claim without creating a queued Pi run when the claim fence changed', async () => {
        const pool = new FakePool();
        pool.queryResults.push([{ id: 9 }], [queueRow({ mode: 'follow_up', claim_epoch: 2 })]);
        const store = new MariadbRuntimeStore(pool);
