...
import { isTerminalRunStateEventType, projectRunState, runStateEventTypeForStatus } from '../agent-runtime/events.js';
import { modelSelectionFromRuntimeConfig } from '../agent-runtime/piHarnessFactory.js';
import { buildPiHarnessResources, PiToolApprovalRequiredError } from '../agent-runtime/piResources.js';
import { OrchestrationService } from './orchestrationService.js';
import { evaluateToolPermission, toolPermissionRules } from './toolPermissionPolicy.js';
import { composeTaggedUserContent, ContextTagRegistry, normalizeContextTags } from './contextTags.js';
import { RuntimeDomainError } from './runtimeErrors.js';
import { classifyRuntimeError, runtimeErrorPublicPayload } from './runtimeErrorClassifier.js';
import { createMcpClient, resolveMcpClientConfigSecrets } from './mcpStreamableHttpClient.js';
import { createDefaultChatModelClient, ModelProviderError } from './modelProvider.js';
...
import { assertReadOnlyAllowsOperation, normalizeSubagentMode, resolveSubagentMode } from './subagentMode.js';
...
function sessionQueueMode(value) {
    if (value === 'steer' || value === 'follow_up' || value === 'stop_and_run')
        return value;
    throw new RuntimeDomainError('queue_mode_invalid', 'Queue mode must be steer, follow_up, or stop_and_run');
}
function sessionQueueAttachments(value) {
    if (value === undefined)
        return [];
...
function sessionQueueItemPayload(item) {
    return {
        mode: item.mode,
...
function boundedReadableIDSegment(value, fallback, maxLength = 32) {
...
function readableIDSegment(value, name, maxLength = 48) {
    const normalized = asString(value)
        .trim()
        .toLowerCase()
        .replace(/[^a-z0-9_.-]+/g, '-')
...
function sessionBusinessID(workspaceID, sessionID) {
    return `s_w${base36(workspaceID)}_${seq36(sessionID)}`;
}
function fallbackSessionTitle(timestamp) {
...
function sanitizeGeneratedSessionTitle(value) {
...
function deterministicSessionTitleFromUserContent(userContent) {
    return sanitizeGeneratedSessionTitle(userContent)
        .replace(/^(帮我|请帮我|请|麻烦|麻烦你)\s*/i, '')
        .trim();
}
function sessionTitleRequestContent(userContent) {
    return [
...
function runBusinessID(workspaceID, sessionID, runID) {
    return `r_w${base36(workspaceID)}_${seq36(sessionID)}_${seq36(runID)}`;
}
function workspaceIDFromRuntimeRunID(runtimeRunID) {
...
function workspaceIDFromArtifactID(artifactID) {
...
function entryRuntimeIdempotencyKey(sessionID, runtimeRunID, role, entrySeq) {
    return runtimeIdempotencyKey('entry', sessionID, 'runtime', runtimeRunID, role, seq36(entrySeq));
}
function requireClientID(field, value) {
...
const ACTIVE_RUN_STATUSES = new Set(['queued', 'running', 'awaiting_decision', 'awaiting_input']);
const TERMINAL_RUN_STATUSES = new Set(['completed', 'failed', 'cancelled', 'timeout', 'interrupted']);
function hasAwaitingDecisionAction(actions) {
    return actions.some((action) => asString(action.status) === 'awaiting_decision');
}
function isApprovingDecision(decision) {
    return decision === 'approve_once' || decision === 'approve_session';
}
function parsePiApprovalDecision(payload) {
    const raw = firstString(payload.decision, payload.action).toLowerCase();
    if (raw === 'approve_once' || raw === 'approve_session' || raw === 'reject' || raw === 'steer')
        return raw;
...
function runStateEventPayload(runtimeRunID, status, code = '', message = '') {
    return {
...
function runtimeIdempotencyKey(prefix, ...parts) {
...
function queuedRunStartOutcome(value) {
...
function textFromRuntimeEvents(events) {
    ...
    const streamed = events
        ...
        .join('');
    if (streamed && !isApprovalWaitNarrationText(streamed))
        return streamed;
...
function piApprovalFromPermissionEvent(event) {
...
function pendingPiApprovalsFromRuntimeEvents(events) {
    ...
    for (const event of events.map(asRecord)) {
        if (event.type === 'permission.asked') {
            const approvalID = firstString(event.request_id, event.approval_id);
            const callID = firstString(event.call_id, event.tool_call_id, event.provider_tool_call_id);
            for (const id of [approvalID, callID].filter(Boolean))
                pendingByID.set(id, event);
            continue;
        }
    ...
    return [...new Set(pendingByID.values())].map(piApprovalFromPermissionEvent).filter((approval) => approval !== null);
}
function isApprovalRequiredToolFailureEvent(event) {
...
function isPiApprovalBypassBoundaryEvent(event, hasApprovalResolution) {
    if (['session.step.started', 'model.call_started', 'model.continuation_started', 'provider.call_started', 'provider.continuation_started'].includes(firstString(event.type)))
        return true;
...
function hasPiApprovalRequestEvents(events) {
    return events.map(asRecord).some((event) => event.type === 'permission.asked');
}
function piApprovalResolutionResult(event) {
    const result = firstString(event.result, event.decision).toLowerCase();
    if (result === 'approved' || result === 'approve_once' || result === 'approve_session')
        return 'approved';
    if (result === 'rejected' || result === 'reject')
        return 'rejected';
    if (result === 'steer')
        return 'steer';
    return '';
}
function rejectedPiApprovalsFromRuntimeEvents(events) {
    ...
    for (const event of events.map(asRecord)) {
        if (event.type === 'permission.asked') {
            const approvalID = firstString(event.request_id, event.approval_id);
            if (approvalID)
                askedByID.set(approvalID, event);
            continue;
        }
...
function approvedPiApprovalsReadyForExecution(events) {
    const askedByID = new Map();
    ...
    for (const event of events.map(asRecord)) {
        if (event.type === 'permission.asked') {
            const approvalID = firstString(event.request_id, event.approval_id);
            if (approvalID && !askedByID.has(approvalID))
                askedByID.set(approvalID, event);
            continue;
        }
        if (event.type === 'permission.resolved') {
...
function normalizePiAwaitingApprovals(value, fallback) {
...
function withoutPiAwaitingApprovalFields(value) {
    const { awaiting_approval: _awaitingApproval, awaiting_approvals: _awaitingApprovals, ...rest } = asRecord(value);
    return rest;
}
function runtimeEventsUntilPendingPiApprovals(events, awaitingApprovals = []) {
    ...
    if (approvalIDs.size === 0 && callIDs.size === 0)
        return events;
    let approvalIndex = -1;
    events.forEach((event, index) => {
        const record = asRecord(event);
        if (record.type !== 'permission.asked')
            return;
...
function profileSnapshotHash(profile) {
...
function normalizedResourceIDSet(values) {
    const set = new Set();
    for (const value of values) {
...
function resourceRefConfig(ref) {
    return isRecord(ref) ? asRecord(ref.config) : {};
}
function refMatchesResource(ref, resource) {
    if (typeof ref.resource_id === 'number')
        return resource.id === ref.resource_id;
...
async function resourcesForProfile(_store, _workspaceID, profile) {
    if (!profile.frozen_resources) {
        throw new RuntimeDomainError('runtime_profile_resources_not_frozen', 'Runtime profile resources are not frozen', 409);
    }
    return [
...
function coerceStructuredOutput(text, schema) {
...
function valueTypeMatches(expected, value) {
    switch (expected) {
        case 'string':
...
function validateAgainstSchema(schema, value, path = '$') {
    ...
    if (schemaType === 'object' || isRecord(value)) {
        ...
        for (const [key, propertySchema] of Object.entries(properties)) {
            if (!(key in record) || !isRecord(propertySchema))
                continue;
...
function redactPublicRuntimeConfig(value) {
    if (Array.isArray(value))
        return value.map((item) => redactPublicRuntimeConfig(item));
    if (!isRecord(value))
        return value;
    const output = {};
...
function isPersistedSecretReferenceKey(key) {
    return /(^|_)(credential|resource)?_?(id|ref)$/.test(key) || /(_env|_var)$/.test(key) || key === 'env';
}
function containsResolvedSecret(value, credentialContext = false) {
    if (Array.isArray(value))
        return value.some((item) => containsResolvedSecret(item, credentialContext));
    if (!value || typeof value !== 'object')
        return false;
    for (const [rawKey, child] of Object.entries(value)) {
        const key = rawKey.toLowerCase();
        if (isPersistedSecretReferenceKey(key) && (typeof child === 'string' || typeof child === 'number'))
            continue;
        ...
        if (sensitive && child !== undefined && child !== null && String(child).trim())
            return true;
        if (containsResolvedSecret(child, nestedCredentialContext))
            return true;
    }
    return false;
}
function assertUnresolvedProfileSnapshot(profile) {
    if (containsResolvedSecret(profile)) {
        throw new RuntimeDomainError('provider_credential_reference_required', 'Provider and MCP credentials must use an unresolved credential reference', 400);
    }
}
function publicMcpResource(resource) {
    return {
        ...publicResource(resource),
        spec: redactPublicRuntimeConfig(resource.spec),
        endpoint: redactPublicRuntimeConfig(resource.endpoint)
    };
}
...
function normalizeRuntimeToolCall(value, index = 0) {
...
function actionToolCallID(action) {
...
function actionOperationType(action) {
...
function toolResultTextValues(value, depth = 0) {
    if (depth > 4 || value == null)
        return [];
    if (typeof value === 'string')
        return [value];
    if (Array.isArray(value))
        return value.flatMap((item) => toolResultTextValues(item, depth + 1));
...
function isInternalRuntimeEventPersistenceText(text) {
    if (!text)
        return false;
    if (text.includes('uk_ai_agent_runtime_events_session_seq'))
        return true;
    if (text.includes('Duplicate entry') && text.includes('ai_agent_runtime_events'))
        return true;
    const normalized = text.toLowerCase();
    return text.includes('Duplicate entry') && (text.includes('服务端日志写入') ||
        (normalized.includes('runtime event') && (normalized.includes('persist') || normalized.includes('logging'))));
}
function isInternalRuntimeEventPersistenceEvent(event) {
    const record = asRecord(event);
    ...
    if (INTERNAL_RUNTIME_EVENT_FAILURE_TYPES.has(type)) {
        ...
        if (constraint === 'uk_ai_agent_runtime_events_session_seq')
            return true;
        if (table === 'ai_agent_runtime_events' && code === 'ER_DUP_ENTRY')
            return true;
        const texts = [
            firstString(record.message),
            firstString(error.message, typeof record.error === 'string' ? record.error : ''),
    ...
    if (!INTERNAL_RUNTIME_EVENT_NARRATION_TYPES.has(type))
        return false;
...
function runtimeEventNarrationSpan(event) {
...
function runtimeEventNarrationText(event) {
    const record = asRecord(event);
    return [record.text, record.delta, record.message]
        .map((value) => firstString(value))
        .filter(Boolean)
...
function runtimeEventPersistenceHiddenFlags(events) {
...
function summarizeRuntimeEventsForEntry(events) {
...
function isStreamingDeltaRuntimeEvent(event) {
    const type = firstString(asRecord(event).type, asRecord(event).event);
    return type.endsWith('.delta');
}
function compactRuntimeEventForEntry(event) {
    const record = asRecord(event);
    const type = firstString(record.type, record.event);
    ...
    for (const key of ['result', 'structured', 'output', 'content', 'data', 'tool_result', 'raw']) {
        if (key in next)
            next[key] = compactToolResultValue(next[key]);
    }
    return next;
}
...
function projectRuntimeEventsForEntry(events, run) {
    if (!run || run.status !== 'awaiting_decision')
        return events;
    ...
    return runtimeEventsUntilPendingPiApprovals(events, awaitingApprovals);
}
function projectEntryForList(entry, run) {
    ...
    const projectedContent = (!entry.content.trim() || currentContentIsInternalPlan)
        ? firstString(runText && !isInternalContinuationPlan(runText) ? runText : '', toolResults.length > 0 ? toolResultsFallbackSummary(toolResults) : '')
        : '';
    ...
    if (!Array.isArray(projectedOutput.runtime_events))
...
function isInternalContinuationPlan(text) {
...
function prepareActionContinuationAnswer(assistantContent, completion, fallbackSummary, emitSynthesizedEvent = false, answerElapsedMs = 0) {
    if (assistantContent.trim() && !isInternalContinuationPlan(assistantContent)) {
        return { assistantContent, completion, events: [] };
    }
...
function actionResultSummary(action, toolName, toolStatus, toolContent, toolOutput) {
...
function eventBusinessID(runtimeRunID, eventSeq) {
    return `${runtimeRunID}:ev${seq36(eventSeq)}`;
}
function runtimeRunCoordinate(runtimeRunID) {
...
function actionBusinessID(action) {
    return actionBusinessIDFor(action.runtime_run_id, action.id);
}
function actionBusinessIDFor(runtimeRunID, actionID) {
...
function mcpToolCallRequestID(runtimeRunID, toolName, actionInternalID) {
    const coordinate = runtimeRunCoordinate(runtimeRunID);
...
function mcpToolsListRequestID(resource) {
...
function mcpServerScopedRequestID(resource, serverKey) {
    const base = mcpToolsListRequestID(resource);
    if (!serverKey || normalizeResourceID(serverKey) === normalizeResourceID(resource.resource_key))
        return base;
    return `${base}_${boundedReadableIDSegment(serverKey, 'mcp-server')}`;
}
function actionBusinessIDFromEventPayload(payload) {
...
function eventDisplayJSON(eventType, payload) {
    const action = asRecord(payload.action);
    ...
    const actionLabel = firstString(payload.title, actionDisplayJSON.title, actionDisplayJSON.name, action.capability_id, action.action_kind);
    const stage = firstString(payload.stage);
    const elapsed = payload.elapsed_ms === undefined ? undefined : Number(payload.elapsed_ms);
    const status = firstString(payload.status, action.status, eventType === 'action.decision_required' ? 'awaiting_decision' : '', eventType === 'tool.executed' ? 'succeeded' : '', eventType === 'tool.failed' ? 'failed' : '', eventType === 'tool.rejected' ? 'rejected' : '', eventType === 'session.step.failed' || eventType === 'session.error' ? 'failed' : '', eventType === 'session.compaction.started' ? 'running' : '', eventType === 'session.compaction.ended' ? 'success' : '');
    ...
    const childStatus = firstString(status, eventType === 'subagent.spawned' ? 'running' : '', eventType === 'subagent.started' ? 'running' : '', eventType === 'subagent.progress' ? 'running' : '', eventType === 'subagent.blocked_approval' ? 'blocked_approval' : '', eventType === 'subagent.completed' ? 'completed' : '', eventType === 'subagent.failed' ? 'failed' : '', eventType === 'subagent.cancelled' ? 'cancelled' : '', eventType === 'subagent.interrupted' ? 'interrupted' : '', eventType === 'subagent.timeout' ? 'timeout' : '');
    return {
        event_type: eventType,
        title: firstString(payload.title, eventType.startsWith('subagent.') ? firstString(payload.name, actionDisplayJSON.name, actionDisplayJSON.title) : '', eventType === 'run.step_announced' && stage ? `步骤 ${stage}` : '', eventType === 'action.decision_required' && actionLabel ? `需要确认 ${actionLabel}` : '', eventType === 'action.execution_started' && actionLabel ? `开始执行 ${actionLabel}` : '', eventType === 'action.execution_succeeded' && actionLabel ? `已执行 ${actionLabel}` : '', eventType === 'action.execution_failed' && actionLabel ? `执行失败 ${actionLabel}` : '', eventType === 'tool.executed' && actionLabel ? `已执行 ${actionLabel}` : '', eventType === 'tool.failed' && actionLabel ? `执行失败 ${actionLabel}` : '', eventType === 'tool.rejected' && actionLabel ? `已拒绝 ${actionLabel}` : '', eventType),
...
function publicActionEvent(event) {
    return {
...
function storedActionEventRuntimeRecord(event) {
    return {
        type: event.event_type,
        persisted: true,
        payload: {
...
function publicPiRuntimeEvent(event, run, index) {
...
function publicPiRuntimeEvents(run, afterEventID = '', eventSource) {
    ...
    const projectedEvents = runtimeEvents
        .map((source, index) => ({
        hidden: hidden[index],
        event: publicPiRuntimeEvent(source, run, index)
    }))
...
function publicRuntimeArtifact(artifact) {
    return {
...
function normalizeRuntimeToolCalls(values) {
    const calls = [];
    const seen = new Set();
    for (const [index, value] of values.entries()) {
        ...
        if (seen.has(call.tool_call_id) || seen.has(semanticKey))
            continue;
...
function runtimeToolDefinitionsByName(values = []) {
    const output = new Map();
    for (const value of values) {
        if (!value?.name)
            continue;
        const qualified = `${firstString(value.mcp_server_key)}:${value.name}`;
        if (!output.has(qualified))
            output.set(qualified, value);
...
function applyToolDefinitionMetadata(call, definitions) {
...
function isToolCallPayloadText(text) {
    const parsed = extractJSONFromText(text);
    if (!parsed)
        return false;
    return Array.isArray(parsed.tool_calls) || isRecord(parsed.tool_call);
}
function buildToolResultContinuationContent(toolResults) {
    return `tool_result:\n${JSON.stringify({
...
function hasUsableModelOutput(completion) {
    ...
    return Boolean((typeof completion?.text === 'string' && completion.text.trim()) ||
        (Array.isArray(completion?.tool_calls) && completion.tool_calls.length > 0) ||
        hasUsablePart);
}
function toolPermissionForCall(profile, call, actorRole = '') {
...
function effectiveProfileToolPolicy(profile, call) {
    ...
    const resourceRules = (profile.frozen_resources?.mcp_servers || []).flatMap((resource) => {
        if (call && !runtimeToolCallMatchesMcpResource(call, resource))
            return [];
...
function mcpRefMatchesToolCall(ref, call) {
    const callServerIDs = normalizedResourceIDSet([
        call.mcp_server_key,
        call.mcp_server_id
    ]);
    if (callServerIDs.size === 0)
        return true;
    return callServerIDs.has(normalizeResourceID(ref.resource_id));
}
function scopedMcpPermissionRule(rule, ref, call, scope) {
    const serverKeys = [
        call?.mcp_server_key,
        call?.mcp_server_id,
        ref.resource_id
...
function scopedMcpResourcePermissionRule(rule, resource, call, scope) {
    const serverKeys = [
        call?.mcp_server_key,
        call?.mcp_server_id,
...
function actionKindForToolCall(call) {
    const toolName = call.tool_name.toLowerCase();
    if (toolName === 'easydo_pipeline_trigger' ||
...
function executorTypeForActionKind(actionKind) {
    if (actionKind === 'pipeline.trigger')
        return 'pipeline';
    if (actionKind === 'subagent.spawn')
        return 'subagent';
    return 'mcp';
}
function actionSourceForToolCall(_call) {
    return 'model';
}
function actionTargetForToolCall(call) {
...
function actionPolicyForToolCall(call, permission) {
...
function riskLevelForOperation(operationType) {
    const normalized = operationType.toLowerCase();
    if (!normalized || normalized === 'unknown')
        return 'write';
    if (['delete', 'destructive', 'drop', 'destroy'].includes(normalized))
        return 'destructive';
    if (['admin', 'impersonate', 'grant', 'revoke'].includes(normalized))
        return 'admin';
    if (['network', 'http', 'fetch'].includes(normalized))
        return 'network';
    if (['write', 'create', 'update', 'mutate', 'mutation', 'execute', 'deploy', 'run'].includes(normalized))
        return 'write';
    return 'read';
}
function actionDisplayForToolCall(call, actionKind, policy, target) {
...
function runtimeToolCallFromAction(action) {
    return {
...
function easyDoRuntimeDeveloperInstructions(baseInstructions, tools = []) {
...
function modelRequestEventPayload(request, options) {
    ...
    const contextWindow = firstPositiveNumber(binding.context_window_tokens, binding.context_window, model.context_window_tokens, model.context_window, model.contextWindow, model.context_tokens, provider.context_window, provider.contextWindow);
    const maxTokens = firstPositiveNumber(binding.max_output_tokens, model.max_output_tokens, model.max_tokens, model.maxTokens, inference.max_tokens, inference.maxTokens, inference.max_output_tokens);
    const thinkingLevel = firstString(inference.thinking_level, inference.thinkingLevel, inference.reasoning_effort, inference.reasoningEffort, inference.reasoning, model.thinking_level, model.thinkingLevel, model.reasoning);
    return {
        phase: options.phase,
...
function modelCompletionEventPayload(completion, options) {
...
function toolCallEventPayload(call) {
    return {
...
function toolResultEventPayload(toolResult) {
    return {
...
function normalizeMcpToolDefinition(value, resource) {
    const record = asRecord(value);
    ...
    if (operationType)
        definition.operation_type = operationType;
    if (targetType)
    ...
    if (mcpServerKey)
        definition.mcp_server_key = mcpServerKey;
    const mcpServerID = record.mcp_server_id ?? resource?.id;
    if (mcpServerID !== undefined && mcpServerID !== null && String(mcpServerID).trim())
        definition.mcp_server_id = mcpServerID;
    definition.capability = firstString(record.capability, 'tools');
    return definition;
}
function mcpToolDefinitionsFromValues(values, resource) {
    const definitions = [];
    const seen = new Set();
    for (const value of values) {
...
function uniqueMcpToolDefinitions(values) {
    return mcpToolDefinitionsFromValues(values);
}
function publicMcpToolDefinition(tool) {
    const output = {
        name: tool.name,
    ...
    if (tool.operation_type)
        output.operation_type = tool.operation_type;
    if (tool.target_type)
        output.target_type = tool.target_type;
    if (tool.requires_confirmation === true)
        output.requires_confirmation = true;
    if (tool.mcp_server_key)
        output.mcp_server_key = tool.mcp_server_key;
    if (tool.mcp_server_id !== undefined)
        output.mcp_server_id = tool.mcp_server_id;
    if (tool.capability)
        output.capability = tool.capability;
    return output;
}
function publicPiModelSelection(selection) {
    if (!selection)
        return {};
    return {
        ...
        inference: selection.inference ? Object.fromEntries(Object.entries(selection.inference).filter(([key]) => !/key|token|secret|password/i.test(key))) : undefined
    };
}
function mcpServerConfigs(resource) {
...
function selectMcpServerConfig(resource, call) {
    ...
    if (toolOwners.length > 1 && !wanted) {
        throw new RuntimeDomainError('mcp_tool_ambiguous', `Tool ${call.tool_name} is available from multiple MCP servers; specify mcp_server_key`, 400);
    }
    return configs[0];
}
function resourceSupportsTool(resource, toolName) {
    const tools = discoveredToolNames(resource);
    return tools.length === 0 || tools.includes(toolName);
}
function runtimeToolCallMatchesMcpResource(call, resource) {
...
function mcpClient(resource, config, auth, logger) {
    ...
    if (delegatedToken && !headers.Authorization && !headers.authorization)
        headers.Authorization = delegatedToken;
    const clientConfig = {
        ...resolvedConfig,
...
class McpRuntimeToolExecutor {
    store;
    easydoServerURL;
    logger;
    constructor(store, easydoServerURL = '', logger = runtimeLogger) {
        this.store = store;
        this.easydoServerURL = easydoServerURL;
        this.logger = logger;
    }
    async execute(call, context) {
        ...
        if (!selected)
            throw new RuntimeDomainError('mcp_server_not_callable', 'MCP server endpoint is not callable by easydo-ai-runtime');
    ...
    async selectMcpResource(actor, profile, call, auth) {
        ...
        const callable = (matchingCandidates.length > 0 ? matchingCandidates : candidates).find((resource) => {
            if (resource.status === 'disabled' || resource.status === 'archived')
                return false;
            if (!resourceSupportsTool(resource, call.tool_name))
                return false;
...
function currentSessionKey(actor, payload) {
    const profileSelection = asRecord(payload.profile_selection);
    const selectedProfileID = firstString(profileSelection.profile_id, profileSelection.agent_profile_id, payload.profile_id, payload.agent_profile_id);
    if (selectedProfileID) {
        const selectedVersionKey = firstString(profileSelection.profile_version_id, profileSelection.agent_profile_version_id, payload.profile_version_id, payload.agent_profile_version_id, 'latest');
...
function isLatestProfileSelection(value) {
    return asString(value).trim().toLowerCase() === 'latest';
}
function isDraftProfileSelection(value) {
    return asString(value).trim().toLowerCase() === 'draft';
}
function profileVersionKey(value) {
    const raw = asString(value).trim();
    return raw || 'latest';
}
function assertSnapshotCallable(profile) {
...
function enforceChatboxInputLimit(profile, content) {
    ...
    if (estimateInputTokens(content) > hardLimit) {
        throw new RuntimeDomainError('chatbox_input_too_large', `Input exceeds Agent Profile Max Tokens / 2 hard limit (${hardLimit} tokens)`, 400);
    }
}
const RUNNING_MODEL_SWITCH_STATUSES = new Set(['queued', 'running']);
function normalizeSessionModelOverride(value) {
...
function hasSessionModelOverride(override) {
...
function mergeSessionModelOverride(profile, override) {
    if (!hasSessionModelOverride(override))
        return profile;
    return {
        ...profile,
...
export class SessionService {
    ...
    agentRuntimeEngine;
    toolExecutor;
    piApprovalLocks = new Map();
    lastOrphanReconcileAt = 0;
    logger;
    constructor(store, profileService, chatModelClient = createDefaultChatModelClient(), toolExecutor, options = {}, contextTagRegistry = new ContextTagRegistry(), agentRuntimeEngine) {
        this.store = store;
        this.profileService = profileService;
        this.chatModelClient = chatModelClient;
        this.options = options;
        this.contextTagRegistry = contextTagRegistry;
    ...
    async appendSessionQueueEvent(input) {
    ...
    async appendUncommittedQueueEvent(result) {
        if (!result.event || result.event_committed)
            return;
        await this.agentRuntimeEngine?.eventStore.append(result.event);
    }
    runLogContext(run, operation) {
        return {
    ...
    async drainSessionQueueAfterTerminalRun(run) {
        ...
        while (guard < SESSION_QUEUE_ACTIVE_ITEM_LIMIT) {
            ...
            if (result.applied + result.consumed + result.failed + result.cancelled_runs === 0)
                break;
            if (result.cancelled_runs > 0 && result.consumed === 0)
                break;
            const active = await this.store.findActiveRun(session.workspace_id, session.id);
            if (active && ACTIVE_RUN_STATUSES.has(active.status) && result.consumed === 0 && result.applied === 0)
                break;
        }
    }
    leaseExpiry() {
    ...
    async runPiHarnessWithLease(run, input) {
        ...
        try {
            ...
            const queueActor = session
                ...
                : undefined;
            const onSafeCheckpoint = queueActor && session
                ? async () => {
    ...
    async reconcileExpiredRunOwners(force = false) {
        ...
        if (!force && currentTime - this.lastOrphanReconcileAt < throttleMs)
            return [];
    ...
    async withPiApprovalLock(actor, runtimeRunID, operation) {
    ...
    providerEmptyOutputDetails(source, attempt) {
    ...
    providerContinuationRequest(request, details) {
        return {
            ...request,
            content: [
    ...
    async completeWithBoundedContinuation(request, options = { phase: 'initial', round: 1 }) {
        let firstEmpty = null;
        const startedEvent = {
        ...
        try {
            ...
            throw new ModelProviderError('provider_response_empty', 'Provider response did not include assistant text, refusal, warning, or tool calls', 502, {
            ...
            if (error instanceof ModelProviderError && error.code === 'provider_response_empty') {
                if (!('first_attempt' in error.details)) {
                    throw new ModelProviderError(error.code, error.message, error.status, {
                        first_attempt: firstEmpty,
    ...
    async *streamBoundedProviderContinuation(request, options = { phase: 'fallback', round: 2 }) {
    ...
    async getCurrentSession(actor, payload) {
    ...
    async resolveProfileForSession(actor, payload) {
        const profileSelection = asRecord(payload.profile_selection);
        const selectedProfileID = Number(firstString(profileSelection.profile_id, profileSelection.agent_profile_id, payload.profile_id, payload.agent_profile_id) || 0);
        const selectedProfileVersion = profileSelection.profile_version_id ??
    ...
    async resolveProfileForRun(actor, session) {
    ...
    async profileSnapshotsForNewRun(actor, session) {
    ...
    effectiveContinuationProfile(run) {
    ...
    async assertContinuationRunProfileIntegrity(actor, runOrRuntimeID) {
        ...
        if (!run.profile_snapshot || agentProfileSnapshotHash(run.profile_snapshot) !== run.agent_profile_snapshot_hash) {
            throw new RuntimeDomainError('runtime_profile_snapshot_integrity_failed', 'Runtime run profile snapshot integrity verification failed', 500);
        }
        return run;
    }
    async executionProfileForContinuation(actor, run) {
        return this.resolveProfileSecrets(actor.workspace_id, this.effectiveContinuationProfile(run));
    }
    async updateSessionModelOverride(actor, sessionID, payload) {
        ...
        if (activeRun && RUNNING_MODEL_SWITCH_STATUSES.has(activeRun.status)) {
            throw new RuntimeDomainError('session_model_switch_running', 'Session model can only be switched while the agent is stopped or waiting for user action', 409, {
                active_run: {
        ...
        const effectiveProfile = mergeSessionModelOverride((await this.resolveProfileForRun(actor, { ...session, model_override: undefined })).profile, modelOverride);
    ...
    enqueueSessionTitleGeneration(actor, session, profile, userContent, existingEntryCount) {
    ...
    async generateSessionTitle(actor, session, profile, userContent) {
        try {
            const latest = await this.store.getSession(actor.workspace_id, session.id);
            if (!latest || latest.title_source !== 'fallback')
                return;
            const titleProfile = {
                ...profile,
                inference: {
            ...
            if (!current || current.title_source !== 'fallback')
                return;
    ...
    async recordSessionTitleGenerationError(session, message) {
    ...
    async retryFailedSessionTitleGeneration(actor, session) {
        if (session.title_source !== 'fallback' || !firstString(session.title_generation_error))
            return session;
    ...
    async saveSessionProgress(actor, session, patch) {
        const latest = await this.store.getSession(actor.workspace_id, session.id);
    ...
    async listSessions(actor, payload) {
        const filter = {
    ...
    async archiveSession(actor, sessionID) {
        const session = await this.assertSessionReadable(actor, sessionID, 'active_only');
        const activeRun = await this.store.findActiveRun(actor.workspace_id, session.id);
        if (activeRun && ACTIVE_RUN_STATUSES.has(activeRun.status)) {
            throw new RuntimeDomainError('active_run_conflict', `Session already has active runtime run ${activeRun.runtime_run_id} in status ${activeRun.status}`, 409, {
                active_run: {
                    runtime_run_id: activeRun.runtime_run_id,
                    status: activeRun.status
                }
    ...
    async listEntries(actor, sessionID) {
        ...
        await Promise.all(runtimeRunIDs.map(async (runtimeRunID) => {
            const run = await this.store.getRun(actor.workspace_id, runtimeRunID);
            if (run)
                runs.set(runtimeRunID, run);
        }));
        return hydrated
            .filter((entry) => {
            if (entry.role !== 'assistant' || !entry.runtime_run_id)
                return true;
            const run = runs.get(entry.runtime_run_id);
            if (!run?.output_entry_id)
                return true;
            if (run.output_entry_id === entry.id)
                return true;
    ...
    async enqueueSessionQueueItem(actor, sessionID, payload) {
        const session = await this.assertSessionReadable(actor, sessionID, 'active_only');
        const mode = sessionQueueMode(payload.mode);
        const content = asString(payload.content).trim();
        if (!content)
            throw new RuntimeDomainError('session_queue_content_required', 'Queue item content is required');
        ...
        if (result.outcome === 'capacity_exceeded') {
            throw new RuntimeDomainError('session_queue_full', `Session queue already has ${SESSION_QUEUE_ACTIVE_ITEM_LIMIT} active items`, 409, { limit: SESSION_QUEUE_ACTIVE_ITEM_LIMIT });
        }
        if (result.outcome === 'existing' &&
            ...
            throw new RuntimeDomainError('session_queue_idempotency_conflict', 'client_item_id was already used for a different queue item', 409, { queue_item_id: result.item.queue_item_id, client_item_id: clientItemID });
        }
    ...
    async drainSessionQueueAfterEnqueue(actor, sessionID, item) {
        const activeRun = await this.store.findActiveRun(actor.workspace_id, sessionID);
        const canApplySteerNow = Boolean(item.mode === 'steer'
            && activeRun
    ...
    scheduleSessionQueueDrain(actor, sessionID) {
    ...
    async listSessionQueueItems(actor, sessionID) {
        await this.assertSessionReadable(actor, sessionID, 'active_or_inactive');
    ...
    async cancelSessionQueueItem(actor, sessionID, queueItemID) {
        ...
        if (!normalizedQueueItemID)
            throw new RuntimeDomainError('session_queue_item_id_required', 'queue_item_id is required');
    ...
    async reorderSessionQueueItems(actor, sessionID, payload) {
        await this.assertSessionReadable(actor, sessionID, 'active_only');
        ...
        if (result.outcome === 'conflict') {
            throw new RuntimeDomainError('session_queue_reorder_conflict', 'Queue order no longer matches the current pending item set', 409);
        }
        return { items: result.items };
    }
    async processSessionQueue(actor, sessionID, options = {}) {
        const session = await this.assertSessionReadable(actor, sessionID, 'active_only');
        ...
        if (item.mode === 'follow_up') {
            ...
            if (started.queue_start_outcome === 'already_consumed')
                return summary;
    ...
    async applyQueueSteerItem(actor, session, activeRun, item, options) {
        ...
        if (applied.outcome === 'applied' || applied.outcome === 'already_applied') {
            if (applied.outcome === 'applied') {
                ...
                await this.agentRuntimeEngine?.runner.steer?.(activeRun.runtime_run_id, item.queue_item_id, item.content);
            }
    ...
    async commitClaimedQueueLifecycle(item, claimedItem, lifecycleEvent) {
    ...
    queueSteerAppliedResult(run, item, timestamp) {
    ...
    async startQueuedFollowUpRun(actor, session, item) {
        const payload = {
            content: item.content,
    ...
    async hydrateEntryActions(workspaceID, entries) {
        ...
        if (internalIDs.size === 0 && businessIDs.size === 0)
            return entries;
        const liveActions = new Map();
        const liveActionsByBusinessID = new Map();
        ...
        if (liveActions.size === 0 && liveActionsByBusinessID.size === 0)
            return entries;
    ...
    async assertSessionCanAcceptEntry(actor, sessionID, payload) {
        await this.assertSessionReadable(actor, sessionID, 'active_only');
        const content = asString(payload.content).trim();
        if (!content) {
    ...
    async assertSessionHasNoActiveRun(actor, session) {
        const activeRun = await this.store.findActiveRun(actor.workspace_id, session.id);
        if (!activeRun || !ACTIVE_RUN_STATUSES.has(activeRun.status))
            return;
    ...
    async persistRuntimeEvents(actor, session, runtimeRunID, events) {
        const persisted = [];
        ...
        for (const event of events) {
            ...
            if (actionID)
                storedPayload.action_id = actionID;
            if (entryID)
                storedPayload.entry_id = entryID;
    ...
    async listRunEvents(actor, runtimeRunID, afterEventID = '') {
        return this.listRunEventsScoped(actor, runtimeRunID, {}, afterEventID);
    }
    async listRunEventsScoped(actor, runtimeRunID, payload = {}, afterEventID = '') {
    ...
    async listRunEventsByRuntimeID(runtimeRunID, afterEventID = '') {
        const workspaceID = workspaceIDFromRuntimeRunID(runtimeRunID);
        if (!workspaceID) {
            throw new RuntimeDomainError('runtime_run_id_invalid', 'Runtime run id is invalid');
        }
    ...
    async *streamRunEvents(actor, runtimeRunID, payload = {}) {
        ...
        const unsubscribe = this.agentRuntimeEngine.eventStore.subscribe(runtimeSessionID, (event) => {
            if (firstString(event.runtime_run_id) !== runtimeRunID)
                return;
            notified = true;
            wake?.();
            wake = undefined;
    ...
    async replayPiRuntimeEventsForRun(run) {
        if (!this.agentRuntimeEngine) {
            throw new RuntimeDomainError('pi_runtime_engine_unavailable', 'Pi runtime engine is not configured', 500);
        }
    ...
    truncatePiRuntimeEventsAtPendingApproval(run, events) {
    ...
    async getRuntimeArtifactByID(artifactID) {
    ...
    async getRuntimeArtifact(actor, artifactID, payload = {}) {
        const workspaceID = workspaceIDFromArtifactID(artifactID);
        if (!workspaceID) {
            throw new RuntimeDomainError('runtime_artifact_id_invalid', 'Runtime artifact id is invalid');
        }
    ...
    async assertParentVisibleArtifactAccess(actor, artifact, payload) {
    ...
    async assertChildRunLinkAccess(actor, link, payload, childSessionID, childRuntimeRunID) {
        const childRunLinkID = firstString(payload.child_run_link_id);
        const parentRuntimeRunID = firstString(payload.parent_runtime_run_id);
        const parentActionID = firstString(payload.parent_action_id);
        if (!link ||
    ...
    async saveRuntimeArtifact(params) {
    ...
    async persistProviderRawArtifact(params) {
    ...
    async persistProviderRawFromCompletion(actor, session, runtimeRunID, completion, providerCallIndex = 1) {
    ...
    async createToolResultRecord(params) {
    ...
    approvalRequestForAction(action, expiresAt) {
        const input = actionInput(action);
        const policy = actionPolicy(action);
        const target = actionTarget(action);
        const display = asRecord(action.display_json);
        const mcpServerKey = firstString(actionMcpServerKey(action), input.mcp_server_key, policy.mcp_server_key, display.mcp_server_key);
    ...
    async findMatchingSessionGrant(params) {
        ...
        return grants.find((grant) => grant.status === 'active' &&
    ...
    async findMatchingPiSessionGrant(params) {
        const nowMs = Date.now();
        const grants = await this.store.listSessionPermissionGrants(params.actor.workspace_id, params.session.id);
        return grants.find((grant) => grant.status === 'active' &&
            Date.parse(grant.expires_at) > nowMs &&
            ...
            grant.executor_type === 'mcp' &&
            grant.resource_type === params.resource.resource_kind &&
            grant.resource_id === params.resource.resource_key);
    }
    async createPiSessionPermissionGrant(params) {
        ...
        const active = existing.find((grant) => grant.status === 'active' &&
    ...
    eventRecorder(target, emit) {
        return (type, payload = {}) => {
            ...
            if (emitted && typeof emitted.then === 'function') {
                return emitted.then((result) => {
                    applyPersisted(result);
                    return result;
                });
            }
            const emittedValue = emitted;
            applyPersisted(emittedValue);
            return emittedValue;
        };
    }
    async resolveSubagentProfiles(_workspaceID, profile) {
        if (profile.frozen_resources) {
        ...
        throw new RuntimeDomainError('runtime_profile_subagents_not_frozen', 'Runtime profile Subagents are not frozen', 409);
    }
    builtinEasyDoCapabilityResource(actor, auth) {
    ...
    async discoverMcpTools(resource, auth, record) {
        ...
        for (const entry of configs) {
            ...
            try {
                ...
                discovered.push(...mcpToolDefinitionsFromValues(tools.map((tool) => ({ ...asRecord(tool), mcp_server_key: serverKey })), resource));
            }
            catch (error) {
    ...
    skillShouldLoad(skill, ref, content) {
        const config = resourceRefConfig(ref);
        if (config.auto_load === true || config.required === true)
            return true;
    ...
    async prepareExecutionContext(actor, session, profile, content, payload, runtimeRunID, auth, emit) {
        const runtimeEvents = [];
        ...
        const mcpTools = uniqueMcpToolDefinitions((await Promise.all(mcpResources.map(({ resource }) => this.discoverMcpTools(resource, auth, record)))).flat());
        const capabilities = {
        ...
        return {
            ...
            runtimeEvents,
            outputSchema,
            developerInstructions: easyDoRuntimeDeveloperInstructions(firstString(asRecord(profile.prompt).developer, asRecord(profile.prompt).developer_instructions, asRecord(profile.prompt).instructions), mcpTools),
            mcpTools
        };
    }
    async runSubagentTasks(actor, session, parentProfile, content, payload, contextRef, subagentProfiles, runtimeRunID, record) {
        ...
        for (const [index, rawTask] of limitedTasks.entries()) {
            ...
            try {
                ...
                if (subagentOutcome === 'completed')
                    this.logger.info(logInput);
                else
    ...
    async runOrchestratedSubagentTasks(params) {
        const results = [];
        const actions = [];
        ...
        await orchestration.assembleAndRun({
            ...
            executeTask: async (task) => {
                ...
                try {
                    ...
                    if (actionIndex >= 0)
                        actions[actionIndex] = publicAction(nextAction);
                    else
                        actions.push(publicAction(nextAction));
                    if (result.status === 'completed')
                        params.record('subagent.completed', result);
                    else if (result.status === 'blocked_approval')
                        params.record('subagent.blocked_approval', result);
                    else if (result.status === 'cancelled')
                        params.record('subagent.cancelled', result);
                    else
                        params.record('subagent.failed', result);
                    return {
    ...
    async createSubagentSpawnAction(params) {
        const timestamp = now();
        const actionID = await this.store.nextActionId();
    ...
    async executeSubagentChildRun(params) {
        ...
        let execution = await this.store.saveActionExecution({
        ...
        let errorMessage = '';
        let validation = { valid: true, errors: [], structuredOutput: {} };
        const providerRawRefs = [];
        const unsubscribeChildProgress = childEventStore.subscribe(childSessionBusinessID, async (event) => {
            ...
            if (childEventType === 'run.started' || childEventType === 'subagent.started')
                return;
            await params.recordParentEvent?.('subagent.progress', {
                action_id: params.action.action_id,
                action_internal_id: params.action.id,
                task_id: params.taskID,
                agent_profile_id: params.childProfile.id,
        ...
        const artifact = await this.saveRuntimeArtifact({
            ...
            preview: {
                ...
                child_run_link_id: childRunLink.child_run_link_id,
                status: resultStatus,
                summary: assistantContent.slice(0, 500),
                validation_errors: validation.errors
            }
        });
    ...
    async processToolCalls(actor, session, runtimeRunID, profile, toolCalls, existingToolCallIDs = new Set(), auth, emitEvents, toolDefinitions = []) {
        const actions = [];
        ...
        for (const rawCall of normalizeRuntimeToolCalls(toolCalls)) {
            ...
            const semanticKey = toolCallSemanticKey(call);
            if (existingToolCallIDs.has(call.tool_call_id) || existingToolCallIDs.has(semanticKey))
                continue;
            ...
            if (emitEvents)
                await emitEvents([events[events.length - 1]]);
            const timestamp = now();
            const actionID = await this.store.nextActionId();
            ...
            const policyJSON = actionPolicyForToolCall(call, permission);
            if (needsDecision) {
    ...
    async continueAutomaticToolLoop(params) {
        let assistantContent = params.assistantContent;
        let completion = params.completion;
        const agentActions = [];
        const toolResults = [];
        const events = [];
        const existingToolCallIDs = params.existingToolCallIDs || new Set();
        ...
        for (let round = 0; round < 4; round += 1) {
            ...
            const processed = await this.processToolCalls(params.actor, params.session, params.runtimeRunID, params.profile, modelToolCalls, existingToolCallIDs, params.auth, params.emitEvents, params.executionContext.mcpTools);
            agentActions.push(...processed.actions);
            toolResults.push(...processed.toolResults);
            events.push(...processed.events);
            if (processed.pendingActions.length > 0 || processed.toolResults.length === 0)
                break;
            ...
            const continuationOptions = { phase: 'tool_continuation', round: round + 2 };
            ...
            if (params.emitEvents)
                await params.emitEvents([events[events.length - 1]]);
            events.push({
    ...
    async createEntryRun(actor, sessionID, payload, auth) {
        const session = await this.assertSessionReadable(actor, sessionID, 'active_only');
        const content = asString(payload.content).trim();
        if (!content) {
            throw new RuntimeDomainError('entry_content_required', 'Entry content is required');
        }
        const { resolvedProfile, persistedProfileSnapshot, persistedProfileSnapshotHash, executionProfileSnapshot: profileSnapshot } = await this.profileSnapshotsForNewRun(actor, session);
        ...
        if (this.shouldUsePiRuntime(payload)) {
            if (!firstString(payload.queue_item_id))
        ...
        let assistantEntryType = 'message';
        let runStatus = 'completed';
        let errorCode = '';
        let errorMessage = '';
        let agentActions = [...executionContext.agentActions];
        let providerRawRefs = [];
        try {
            const initialModelRequest = {
    ...
    async enrichPiSkillResources(profile, resources, skillNames = []) {
        const selectedSkillIDs = new Set((profile.skills || [])
            .map((ref) => normalizeResourceID(ref.resource_id))
            .filter(Boolean));
        if (selectedSkillIDs.size === 0)
            return resources;
        const requestedSkillIDs = new Set(Array.from(skillNames)
        ...
        return Promise.all(resources.map(async (resource) => {
            ...
            if (!isRequested)
                return resource;
            const spec = asRecord(resource.spec);
            if (firstString(spec.instructions, spec.markdown, spec.content, spec.prompt))
                return resource;
            const repositoryKey = firstString(spec.source_repository);
            const entry = firstString(spec.entry, spec.path);
            const repository = resources.find((candidate) => candidate.resource_kind === 'skill' &&
    ...
    async loadPiSkillResource(profile, resources, resource) {
    ...
    skillContentFromRepositoryScan(repository, skill) {
        if (!repository)
            return '';
        ...
        const match = discovered.map(asRecord).find((item) => normalizeResourceID(item.key) === normalizeResourceID(skill.resource_key) ||
    ...
    async enrichPiMcpResources(actor, profile, resources, auth, record) {
        ...
        for (const ref of mcpRefs) {
            ...
            if (hasResource || normalizeResourceID(ref.resource_id) !== 'easydo')
                continue;
            const builtin = this.builtinEasyDoCapabilityResource(actor, auth);
            if (builtin)
                resourcesWithBuiltin.push(builtin);
        }
        ...
        if (selectedMcpResources.length === 0)
            return resourcesWithBuiltin;
        const discoveredToolsByResource = new Map();
    ...
    selectPiSkillInvocation(content, skills) {
    ...
    async recordPiResourcePreparationEvents(runtimeSessionID, runtimeRunID, profile, summary, content) {
    ...
    async recordPiSkillLoadedEvents(runtimeSessionID, runtimeRunID, summary, loadedSkillNames) {
    ...
    async createPiEntryRun(actor, session, persistedProfileSnapshot, profileSnapshot, resolvedProfile, entries, publicSessionID, content, payload, auth) {
        if (!this.agentRuntimeEngine) {
            throw new RuntimeDomainError('pi_runtime_engine_unavailable', 'Pi runtime engine is not configured', 500);
        }
        const seqBase = entries.length;
        ...
        let run;
        let queueStartOutcome;
        if (queueItemID) {
            const claimedBy = firstString(payload.queue_claimed_by);
            const claimEpoch = Number(payload.queue_claim_epoch || 0);
            const queueMode = firstString(payload.queue_mode) === 'stop_and_run' ? 'stop_and_run' : 'follow_up';
            const followUpStartedEvent = {
                type: 'session.follow_up.started',
                session_id: runtimeSessionID,
                runtime_run_id: runtimeRunID,
            ...
            if (queuedStart.outcome === 'already_consumed') {
                ...
                return {
                    ...
                    user_entry: existingUserEntry,
                    assistant_entry: existingAssistantEntry,
                    queue_start_outcome: 'already_consumed'
                };
            }
            ...
            if (!queuedStart.event_committed)
                await this.agentRuntimeEngine.eventStore.append(queuedStart.follow_up_started_event);
        }
        else {
        ...
        const recordPiPreparationIssue = async (type, eventPayload = {}) => {
            await this.agentRuntimeEngine?.eventStore.append({
                type,
                session_id: runtimeSessionID,
                runtime_run_id: runtimeRunID,
        ...
        const allResources = await this.enrichPiMcpResources(actor, profileSnapshot, await resourcesForProfile(this.store, actor.workspace_id, profileSnapshot), auth, recordPiPreparationIssue);
        ...
        const buildResources = (resources, loadedSkillNames = []) => buildPiHarnessResources({
            profile: profileSnapshot,
            ...
            executeSubagent: async ({ callID, subagent, task, args }) => {
                ...
                if (!match)
                    throw new Error(`Subagent profile ${subagent.id} is not available to this parent profile`);
                ...
                if (result.status === 'blocked_approval' || result.status === 'cancelled')
                    return result;
                await this.agentRuntimeEngine?.eventStore.append({
        ...
        if (loadedSkillNames.length > 0) {
            piResources = buildResources(await this.enrichPiSkillResources(profileSnapshot, allResources, loadedSkillNames), loadedSkillNames);
        }
        const modelConfig = modelSelectionFromRuntimeConfig({
        ...
        const cancelledBeforePrompt = await this.cancelledPiEntryRunResult(actor, session, runtimeRunID, userEntry, streamingAssistantEntry);
        if (cancelledBeforePrompt)
            return cancelledBeforePrompt;
        ...
        const cancelledAfterPrompt = await this.cancelledPiEntryRunResult(actor, session, runtimeRunID, userEntry, streamingAssistantEntry);
        if (cancelledAfterPrompt)
            return cancelledAfterPrompt;
        const rawRuntimeEvents = await this.agentRuntimeEngine.eventStore.replayRun(runtimeRunID);
        ...
        const piTimings = attachActiveDurationTimings({ total_ms: wallMs }, runtimeEvents);
        const assistantEntry = {
    ...
    async cancelledPiEntryRunResult(actor, session, runtimeRunID, userEntry, streamingAssistantEntry) {
        const cancelledRun = await this.store.getRun(actor.workspace_id, runtimeRunID);
        if (!cancelledRun || !['cancelled', 'interrupted'].includes(cancelledRun.status))
            return undefined;
        const currentEntries = await this.store.listEntries(actor.workspace_id, session.id);
        ...
        return {
            ...
            assistant_entry: cancelledAssistantEntry || {
                ...streamingAssistantEntry,
                status: cancelledRun.status === 'cancelled' ? 'cancelled' : 'failed',
    ...
    async finalizePiAwaitingApprovalRun(params) {
    ...
    async finalizeFailedPiEntryRun(params) {
        ...
        const runStatus = errorDescriptor.terminal_status;
        ...
        const embeddedRuntimeEvents = summarizeRuntimeEventsForEntry(runtimeEvents);
        const assistantContent = `Pi 运行失败：${errorDescriptor.user_message}`;
        ...
        const failedWallMs = Number.isFinite(failedStartedMs) && Number.isFinite(failedFinishedMs)
            ...
            : 0;
        const failedTimings = attachActiveDurationTimings({ total_ms: failedWallMs }, runtimeEvents);
        const assistantEntry = {
            ...params.assistantEntry,
            entry_type: 'error',
            status: 'failed',
    ...
    async buildPiContinuationResources(params) {
        ...
        const allResources = await this.enrichPiMcpResources(params.actor, params.profileSnapshot, await resourcesForProfile(this.store, params.actor.workspace_id, params.profileSnapshot), params.auth, recordPiPreparationIssue);
        ...
        const buildResources = (resources, loadedSkillNames = []) => buildPiHarnessResources({
            ...
            executeSubagent: async ({ callID, subagent, task, args }) => {
                ...
                if (!match)
                    throw new Error(`Subagent profile ${subagent.id} is not available to this parent profile`);
                const mode = firstString(args.mode, 'read_only');
                const objective = firstString(task, args.objective, match.profile.description, content);
                const taskID = firstString(args.task_id, args.id, callID);
                const action = await this.createSubagentSpawnAction({
                    actor: params.actor,
        ...
        const loadedSkillNames = skillInvocation?.name ? [skillInvocation.name] : [];
        if (loadedSkillNames.length > 0) {
            piResources = buildResources(await this.enrichPiSkillResources(params.profileSnapshot, allResources, loadedSkillNames), loadedSkillNames);
        }
        return { piResources, skillInvocation };
    }
    async syncParentAfterChildTerminal(actor, childRun, summary) {
        ...
        if (!parentRun || !parentSession || !action)
            return;
        const childCompleted = childRun.status === 'completed';
        const childLinkStatus = childCompleted ? 'completed' : childRun.status === 'cancelled' ? 'cancelled' : 'failed';
        ...
        if (firstString(parentRun.result.runtime_engine, parentRun.request.runtime_engine) === 'pi' && this.agentRuntimeEngine) {
            ...
            const alreadyRecorded = existingEvents.some((event) => String(event.type) === eventType && firstString(asRecord(asRecord(event).payload).child_runtime_run_id) === childRun.runtime_run_id);
            if (!alreadyRecorded) {
                await this.agentRuntimeEngine.eventStore.append({
                    type: eventType,
            ...
            runtimeEvents = summarizeRuntimeEventsForEntry(await this.agentRuntimeEngine.eventStore.replayRun(parentRun.runtime_run_id));
        }
        else {
    ...
    async decidePiApproval(actor, runtimeRunID, payload = {}, auth) {
        return this.withPiApprovalLock(actor, runtimeRunID, () => this.decidePiApprovalLocked(actor, runtimeRunID, payload, auth));
    }
    async decidePiApprovalLocked(actor, runtimeRunID, payload = {}, auth) {
        if (!this.agentRuntimeEngine) {
            throw new RuntimeDomainError('pi_runtime_engine_unavailable', 'Pi runtime engine is not configured', 500);
        }
        ...
        const awaitingApproval = requestedApproval || awaitingApprovals.find((approval) => firstString(approval.approval_id, approval.request_id) === firstString(storedAwaitingApproval.approval_id, storedAwaitingApproval.request_id)) || awaitingApprovals[0] || {};
        ...
        if (isApprovingDecision(decision)) {
            try {
                assertReadOnlyAllowsOperation(subagentModeForRun(run), firstString(awaitingApproval.operation_type, asRecord(awaitingApproval.input).operation_type));
            }
            catch (error) {
                if (!(error instanceof RuntimeDomainError) || error.code !== 'subagent_read_only_violation')
                    throw error;
        ...
        if (!steering && approvalsToExecute.length === 0)
            approvalsToExecute.push(awaitingApproval);
        ...
        for (const approvalToExecute of approvalsToExecute) {
            ...
            if (!approvalCallID || !approvalToolName)
                continue;
            assertReadOnlyAllowsOperation(subagentModeForRun(runningRun), firstString(approvalToExecute.operation_type, approvalInput.operation_type));
            await engine.eventStore.append({
                ...eventBase,
                timestamp: now(),
        ...
        const existingToolResults = Array.isArray(runningRun.result.tool_results)
            ? runningRun.result.tool_results
            : [];
        ...
        const finalStatus = continuationError || hasToolFailure ? 'failed' : 'completed';
    ...
    async *streamPiApproval(actor, runtimeRunID, payload = {}, auth) {
        const startedAt = performance.now();
        if (!this.agentRuntimeEngine) {
            throw new RuntimeDomainError('pi_runtime_engine_unavailable', 'Pi runtime engine is not configured', 500);
        }
        ...
        try {
            while (!result && !runError) {
                while (pendingEvents.length > 0) {
                    yield this.streamEvent('runtime_event', startedAt, { event: pendingEvents.shift() });
                }
                if (result || runError)
                    break;
    ...
    async *streamEntryRun(actor, sessionID, payload, auth) {
        const startedAt = performance.now();
        const session = await this.assertSessionReadable(actor, sessionID, 'active_only');
        const content = asString(payload.content).trim();
        if (!content) {
        ...
        if (existingUserEntry && existingAssistantEntry) {
            ...
            for (const event of replayRuntimeEvents) {
                yield this.streamEvent('runtime_event', startedAt, { event: event, idempotent: true });
            }
        ...
        const livePreparedEvents = [];
        let livePreparedEventNotify = null;
        ...
        const waitForLivePreparedEvent = () => new Promise((resolve) => {
        ...
        let answerStreamMode = 'undecided';
        ...
        try {
            ...
            const initialStreamOptions = { phase: 'initial', round: 1 };
            ...
            if (!assistantContent.trim() && streamToolCalls.length === 0) {
                ...
                const fallbackOptions = { phase: 'fallback', round: 2 };
            ...
            const waitForLiveToolEvent = () => new Promise((resolve) => {
                liveToolEventNotify = resolve;
            });
            let toolLoopCompleted = false;
            const emitLiveToolEvents = async (events) => {
                if (events.length === 0)
                    return;
            ...
            const shouldEmitBufferedAnswer = !streamedAnswerDelta &&
                assistantContent &&
    ...
    async cancelSessionRun(actor, sessionID, payload = {}) {
        const session = await this.assertSessionReadable(actor, sessionID, 'active_only');
        const entries = await this.store.listEntries(actor.workspace_id, session.id);
        ...
        const cancellableEntries = entries.filter((entry) => entry.runtime_run_id === runtimeRunID && ['queued', 'running', 'streaming'].includes(entry.status));
    ...
    async recordPiCancellationEvents(run, timestamp, reason) {
        if (!this.agentRuntimeEngine || firstString(run.result.runtime_engine, run.request.runtime_engine) !== 'pi')
            return [];
        const engine = this.agentRuntimeEngine;
        const runtimeSessionID = firstString(run.result.runtime_session_id, run.request.runtime_session_id);
        if (!runtimeSessionID)
    ...
    async recordPiFailureEvents(runtimeSessionID, runtimeRunID, timestamp, descriptor, requestID = '') {
        if (!this.agentRuntimeEngine || !runtimeSessionID)
            return [];
    ...
    async ensurePiRunStateEvent(run) {
        if (!this.agentRuntimeEngine)
            return [];
        const runtimeSessionID = firstString(run.result.runtime_session_id, run.request.runtime_session_id);
        const eventType = runStateEventTypeForStatus(run.status);
        if (!runtimeSessionID || !eventType)
        ...
        if (isTerminalRunStatus(run.status) && existingTerminal) {
            if (projection.hasTerminalConflict || existingTerminal.type !== eventType) {
                throw new RuntimeDomainError('runtime_run_terminal_event_conflict', `Runtime run ${run.runtime_run_id} is ${run.status} but replay terminal state conflicts`, 500);
            }
            return events;
        }
    ...
    async ensureStoredRunStateEvent(run) {
        const eventType = runStateEventTypeForStatus(run.status);
    ...
    async ensureRunStateEvent(run) {
        if (firstString(run.result.runtime_engine, run.request.runtime_engine) === 'pi') {
            return this.ensurePiRunStateEvent(run);
        }
        return this.ensureStoredRunStateEvent(run);
    }
    async abortPiHarnessRun(run) {
        if (!this.agentRuntimeEngine || firstString(run.result.runtime_engine, run.request.runtime_engine) !== 'pi')
            return false;
        if (typeof this.agentRuntimeEngine.runner.abort !== 'function')
            return false;
        return this.agentRuntimeEngine.runner.abort(run.runtime_run_id);
    }
    continueSessionStream(actor, sessionID, payload, auth) {
    ...
    shouldUsePiRuntime(payload) {
        const requestedEngine = firstString(payload.runtime_engine);
        if (requestedEngine)
            return requestedEngine === 'pi';
        return Boolean(this.agentRuntimeEngine);
    }
    async *continueAfterActionStream(actor, action, decision, auth, sessionGrant) {
        const startedAt = performance.now();
        ...
        const existingEvents = Array.isArray(runningRun.result.runtime_events)
            ? runningRun.result.runtime_events.map((item) => {
                const record = asRecord(item);
                return {
            ...
            : [];
        const runtimeEvents = [...existingEvents];
        const emitRuntimeEvents = async (events) => {
        ...
        for (const item of await emitRuntimeEvents([decisionEvent]))
            yield item;
        if (sessionGrant) {
        ...
        if (approved) {
            ...
            for (const item of await emitRuntimeEvents([toolResultEvent]))
                yield item;
        }
        const toolEntry = {
            id: await this.store.nextEntryId(),
        ...
        const continuationOptions = { phase: 'action_continuation', round: 1 };
        for (const item of await emitRuntimeEvents([
            {
                type: 'tool.result_prepared',
                payload: {
        ...
        try {
            ...
            if (!assistantContent.trim() && streamToolCalls.length === 0) {
                ...
                while (true) {
                    ...
                    for (const item of await emitRuntimeEvents([next.value]))
                        yield item;
                }
                assistantContent = completion?.text || '';
            }
            const fallbackSummary = actionResultSummary(continuedAction, call.tool_name, toolStatus, toolContent, toolOutput);
            const preparedAnswer = prepareActionContinuationAnswer(assistantContent, completion, fallbackSummary, true, elapsedMs(startedAt));
            ...
            if (answerEvents.length > 0) {
                for (const item of await emitRuntimeEvents(answerEvents))
                    yield item;
            }
            for (const item of await emitRuntimeEvents([{
                    type: 'model.continuation_completed',
            ...
            providerRawRefs = [...providerRawRefs, ...providerRaw.refs];
            if (providerRaw.events.length > 0) {
                for (const item of await emitRuntimeEvents(providerRaw.events))
                    yield item;
            }
        }
        catch (error) {
        ...
        for (const item of await emitRuntimeEvents([{
                ...
                payload: runStateEventPayload(runningRun.runtime_run_id, continuationRunStatus, continuationRunStatus === 'failed' ? continuationErrorCode || 'output_schema_invalid' : '', continuationRunStatus === 'failed' ? continuationErrorMessage || outputValidation.errors.join('; ') : '')
            }]))
            yield item;
        const previousAgentActions = Array.isArray(runningRun.result.agent_actions)
            ...
            : [];
        const nextAgentActions = previousAgentActions.map((item) => firstString(item.action_id, item.id) === continuedAction.action_id ? publicAction(continuedAction) : item);
        const assistantEntry = {
            id: await this.store.nextEntryId(),
            session_id: session.id,
            workspace_id: actor.workspace_id,
    ...
    async continueAfterAction(actor, action, decision, auth, sessionGrant) {
        const session = await this.store.getSession(actor.workspace_id, action.session_id);
        if (!session || session.status !== 'active') {
            throw new RuntimeDomainError('ai_session_not_found', 'AI session not found', 404);
        }
        ...
        let continuationErrorCode = '';
        let continuationErrorMessage = '';
        let providerContinuationEvents = [];
        let providerRawRefs = Array.isArray(runningRun.result.provider_raw_refs)
            ? runningRun.result.provider_raw_refs.map((item) => asRecord(item))
            : [];
        try {
    ...
    async persistStepEvent(actor, session, runtimeRunID, stage, start) {
    ...
    async resolveProfileSecrets(workspaceID, profile) {
        const credentialRef = asRecord(profile.provider_credential_ref);
        const existingSecretRef = asRecord(credentialRef.secret_ref);
        if (firstString(credentialRef.api_key, credentialRef.token, credentialRef.bearer_token, credentialRef.env, credentialRef.env_var, existingSecretRef.api_key, existingSecretRef.token, existingSecretRef.bearer_token, existingSecretRef.env, existingSecretRef.env_var)) {
            return profile;
        }
        const credentialID = firstString(credentialRef.credential_id, credentialRef.resource_id, credentialRef.resource_key, credentialRef.id);
        if (!credentialID)
            return profile;
        const provider = asRecord(profile.provider);
        const binding = asRecord(profile.binding);
        const providerID = firstString(provider.provider_id, provider.id, binding.provider_id);
...
function piSessionHistoryFromEntries(entries, excludedRuntimeRunID = '') {
    return entries
...
function canonicalEntryTimestamp(entry) {
...
export class ActionService {
    store;
    sessionService;
    actionDecisionLocks = new Map();
    constructor(store, sessionService) {
        this.store = store;
        this.sessionService = sessionService;
    }
    async decide(actor, actionID, decision, payload = {}) {
    ...
    async decideAndContinue(actor, actionID, decision, payload = {}, auth) {
        return this.withActionDecisionLock(actor, actionID, async () => {
    ...
    async *decideAndContinueStream(actor, actionID, decision, payload = {}, auth) {
    ...
    async withActionDecisionLock(actor, actionID, operation) {
        const key = `${actor.workspace_id}:${actionID}`;
        const previous = this.actionDecisionLocks.get(key) || Promise.resolve();
        let releaseCurrent = () => { };
        const current = new Promise((resolve) => {
    ...
    async getAction(actor, actionID) {
    ...
    async recordActionDecision(actor, action, decision, payload) {
    ...
    async applyAwaitingActionDecision(actor, action, decision, sessionGrant) {
        ...
        if (result.outcome === 'already_decided')
            this.rejectAlreadyDecidedAction(result.action);
        throw new RuntimeDomainError('action_not_found', 'Action not found', 404);
    }
    async createSessionPermissionGrant(actor, action) {
    ...
    rejectAlreadyDecidedAction(action) {
        const existingDecision = this.decisionForStatus(action.status);
        throw new RuntimeDomainError('action_already_decided', `Action already ${isApprovingDecision(existingDecision) ? 'approved' : 'rejected'}`, 409);
    }
    decisionForStatus(status) {
