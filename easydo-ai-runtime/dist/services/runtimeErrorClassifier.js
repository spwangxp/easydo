...
export function classifyRuntimeError(error, context = {}) {
    ...
    if (isTransientTransport(text, httpStatus)) {
        ...
        return descriptor(providerTransport ? stableProviderCode(suppliedCode, 'provider_transport_error') : suppliedCode || `${source}_transport_error`, 'transport', message, providerTransport ? 'The AI provider is temporarily unavailable. Try again.' : 'The AI runtime transport is temporarily unavailable. Try again.', explicitRetryable ?? true, httpStatus || 502, source, 'failed');
    }
...
export function runtimeErrorPublicPayload(descriptor) {
    return {
...
function descriptor(code, category, message, userMessage, retryable, httpStatus, source, terminalStatus) {
    return {
        code,
        category,
        message,
...
function stableProviderCode(code, fallback) {
...
function inferSource(record) {
    ...
    if (code.startsWith('mcp_'))
        return 'mcp';
    if (code.startsWith('provider_') || code.startsWith('openrouter_') || code.startsWith('openai_'))
        return 'provider';
...
function runtimeErrorSource(value) {
...
function isTransientTransport(text, httpStatus) {
    return Boolean((httpStatus && httpStatus >= 500 && httpStatus <= 599) ||
