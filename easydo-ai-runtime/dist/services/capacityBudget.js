...
export function enforceInlineToolResultBudget(value, maxBytes = CAPACITY_BUDGET.max_inline_tool_result_bytes) {
...
export function enforceSseEventBudget(payload, maxBytes = CAPACITY_BUDGET.max_sse_event_bytes) {
...
export function selectRetainedRunEvents(events = [], maxEvents = CAPACITY_BUDGET.max_run_events_retained) {
...
export function paginateEventsByCursor(events = [], options = {}) {
    const pageSize = Math.max(1, Math.min(CAPACITY_BUDGET.replay_page_size, Number(options.page_size) || CAPACITY_BUDGET.replay_page_size));
