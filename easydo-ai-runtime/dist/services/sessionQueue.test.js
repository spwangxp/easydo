...
async function queueFixture(options = {}) {
    ...
    const harnessSteers = [];
    const service = new SessionService(store, profiles, undefined, undefined, { autoDrainSessionQueue: options.autoDrainSessionQueue === true }, undefined, options.runtimeEvents || options.autoDrainSessionQueue
        ? {
            runner: {
                async prompt(input) {
...
describe('SessionService durable queue management', () => {
    ...
    it('applies steer at a safe checkpoint for the bound active run', async () => {
        ...
        const steers = Array.isArray(result.queue_steers) ? result.queue_steers : [];
    ...
    it('applies a claimed steer exactly once and rejects stale claim epochs', async () => {
        ...
        if (claimed.outcome !== 'claimed')
            throw new Error('expected claimed queue item');
    ...
    it('reclaims an expired claim and fences the stale owner epoch', async () => {
        ...
        if (firstClaim.outcome !== 'claimed' || reclaimed.outcome !== 'claimed')
            throw new Error('expected reclaimed queue item');
        await expect(store.applySessionQueueSteer({
            workspace_id: actor.workspace_id,
            session_id: session.id,
            queue_item_id: enqueued.item.queue_item_id,
...
async function seedActiveRun(store, session) {
