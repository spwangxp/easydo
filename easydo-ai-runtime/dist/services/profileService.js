...
import { assertKnownContextTags, assertProfileContextTags, normalizeContextTags, PAGE_ASSISTANT_CONTEXT_TAG, PAGE_ASSISTANT_PROFILE_NAME } from './contextTags.js';
...
function isMaskedSecretRef(value) {
    const record = asRecord(value);
    return record.masked === true || record.configured === true;
}
function assertCredentialReferenceOnly(profile) {
    ...
    if (resolvedValue) {
        throw new RuntimeDomainError('provider_credential_reference_required', 'Provider credentials must use a credential reference', 400);
    }
}
...
function resourceToolPermissions(resource) {
    return asRecord(asRecord(resource.spec).tool_permissions);
}
function refWithDefaultToolPermissions(ref, resource) {
    if (hasRecordFields(asRecord(ref.config).tool_permissions))
        return ref;
...
function defaultPageAssistantProfile(id, actor) {
    const timestamp = now();
    return {
        id,
...
function profileFromPayload(id, actor, payload) {
    const timestamp = now();
    return {
        id,
        workspace_id: actor.workspace_id,
...
function resourceFromPayload(id, actor, payload) {
    const timestamp = now();
...
function mergeResource(resource, payload) {
...
function scanSkillSucceededSpec(resource, discoveredSkills) {
    return {
        ...resource.spec,
...
function scanMcpSucceededSpec(resource, discoveredTools) {
    return {
        ...resource.spec,
...
function normalizeMcpDiscoveredTools(values, serverKey = '') {
    const tools = [];
    const seen = new Set();
    for (const value of values) {
        const record = asRecord(value);
        ...
        if (operationType)
            tool.operation_type = operationType;
        if (targetType)
            tool.target_type = targetType;
        if (record.requires_confirmation === true || record.requiresConfirmation === true)
            tool.requires_confirmation = true;
        if (mcpServerKey)
            tool.mcp_server_key = mcpServerKey;
        tool.capability = firstString(record.capability, 'tools');
        tools.push(tool);
    }
    return tools;
}
async function scanMcpTools(resource) {
    ...
    for (const entry of configs) {
        ...
        try {
            const result = await client.request('tools/list', {}, `mcp_tools_${resource.workspace_id}_${resource.id}_${serverKey}`, { mcp_server_id: serverKey });
...
export async function probeMcpServerConfig(configInput) {
    ...
    try {
        ...
        if (error instanceof RuntimeDomainError)
            throw error;
        throw new RuntimeDomainError('mcp_server_probe_failed', error instanceof Error ? error.message : 'MCP server probe failed', 400);
    }
    finally {
        await client.close();
    }
...
function isResourceRevisionConflict(error) {
    if (error instanceof RuntimeDomainError && error.code === 'agent_resource_revision_conflict')
        return true;
    const message = error instanceof Error ? error.message : String(error || '');
    return /duplicate|already exists/i.test(message);
}
async function assertUniqueResourceKey(store, resource) {
...
function groupResources(resources, profiles) {
    const grouped = {
        mcp_servers: [],
        skills: [],
    ...
    for (const resource of resources) {
        ...
        if (resource.resource_kind === 'skill')
            grouped.skills.push(safeResource);
        if (resource.resource_kind === 'mcp_server')
            grouped.mcp_servers.push(safeResource);
    }
    return grouped;
}
export class AgentProfileService {
    store;
    pageAssistantEnsureJobs = new Map();
    constructor(store) {
        this.store = store;
    }
    async resolveResourceVersion(actor, ref, resource, kind) {
        ...
        if (requestedVersionID > 0) {
            ...
            if (version && requestedSourceVersion) {
                ...
                if (sourceMatches.length > 1) {
                    throw new RuntimeDomainError('agent_profile_resource_version_ambiguous', `${kind} resource ${ref.resource_id} source version is ambiguous`, 409);
                }
                if (sourceMatches[0]?.resource_version_id !== version.resource_version_id) {
                    throw new RuntimeDomainError('agent_profile_resource_version_selector_conflict', `${kind} resource ${ref.resource_id} version selectors conflict`, 409);
                }
            }
        }
        ...
        if (!version || version.resource_id !== resource.id || version.snapshot.resource_kind !== kind) {
            throw new RuntimeDomainError('agent_profile_resource_version_not_found', `${kind} resource ${ref.resource_id} version was not found`, 404);
        }
        if (ref.snapshot_digest && ref.snapshot_digest !== version.snapshot_digest) {
            throw new RuntimeDomainError('agent_profile_resource_digest_mismatch', `${kind} resource ${ref.resource_id} digest does not match its version`, 409);
        }
        return version;
    }
    async resolvePublishedSubagentVersion(actor, ref, profileID) {
        ...
        if (version && requestedVersionID > 0 && requestedLogicalVersion && String(version.version) !== requestedLogicalVersion) {
            throw new RuntimeDomainError('agent_profile_subagent_version_selector_conflict', `Published Subagent ${ref.resource_id} version selectors conflict`, 409);
        }
        if (!version) {
        ...
        if (ref.snapshot_digest && ref.snapshot_digest !== version.snapshot_hash) {
            throw new RuntimeDomainError('agent_profile_subagent_digest_mismatch', `Published Subagent ${ref.resource_id} digest does not match its version`, 409);
        }
        return version;
    }
    ...
    async getProfile(actor, profileID) {
    ...
    async updateProfile(actor, profileID, payload) {
    ...
    async deleteProfile(actor, profileID) {
        ...
        const blockers = dependencies.dependencies.filter((dependency) => ['blocking_live_ref', 'published_version', 'session_reference', 'active_execution'].includes(String(dependency.dependency_class)));
    ...
    async updateResource(actor, resourceID, payload) {
    ...
    async assertResourceStatusMutationAllowed(actor, previous, next) {
        if (previous.status === next.status || !['disabled', 'archived'].includes(next.status))
            return;
    ...
    async deleteResource(actor, resourceID) {
        await this.getResource(actor, resourceID);
        const dependencies = await this.getResourceDependencies(actor, resourceID);
        const blockers = dependencies.dependencies.filter((dependency) => ['blocking_live_ref', 'active_execution'].includes(String(dependency.dependency_class)));
        if (blockers.length > 0) {
    ...
    async getResourceDependencies(actor, resourceID) {
    ...
    async getProfileDependencies(actor, profileID) {
        const profile = await this.getProfile(actor, profileID);
        const profiles = await this.store.listProfiles(actor.workspace_id);
        const dependencies = [];
        ...
        for (const session of await this.store.listSessions(actor.workspace_id)) {
            if (session.agent_profile_id !== profileID)
                continue;
            dependencies.push({
        ...
        return {
            ...
            operations: {
                delete: {
                    allowed: !dependencies.some((item) => ['blocking_live_ref', 'published_version', 'session_reference', 'active_execution'].includes(String(item.dependency_class)))
                },
                disable: {
                    allowed: !dependencies.some((item) => ['blocking_live_ref', 'session_reference', 'active_execution'].includes(String(item.dependency_class)))
                },
                archive: { allowed: true }
            }
        };
    }
    async scanResource(actor, resourceID) {
        const resource = await this.store.getAgentResource(actor.workspace_id, resourceID);
        if (!resource) {
            throw new RuntimeDomainError('agent_resource_not_found', 'Agent resource not found', 404);
        }
        if (resource.resource_kind === 'mcp_server') {
            try {
                ...
                if (error instanceof RuntimeDomainError)
                    throw error;
                throw new RuntimeDomainError('mcp_server_scan_failed', error instanceof Error ? error.message : 'MCP server scan failed', 400);
            }
        }
        try {
            ...
            const updated = await this.saveResourceRevision(actor, {
                ...resource,
                spec: scanSkillSucceededSpec(resource, discoveredSkills),
                updated_at: now()
            }, 'update');
            return {
                resource: sanitizeResource(updated),
            ...
            throw new RuntimeDomainError('skill_repository_scan_failed', error instanceof Error ? error.message : 'Skill repository scan failed', 400);
        }
    }
    async saveResourceRevision(actor, resource, operation) {
    ...
    async publishProfile(actor, profileID, payload) {
        for (let attempt = 0; attempt < 3; attempt += 1) {
            ...
            try {
                return await this.store.publishProfileVersion({ ...profile, status: 'active', updated_at: now() }, version, profile.updated_at);
            }
            catch (error) {
                if (!(error instanceof RuntimeDomainError) || error.code !== 'agent_profile_publish_conflict' || attempt === 2)
                    throw error;
            }
        }
        throw new RuntimeDomainError('agent_profile_publish_conflict', 'Agent profile changed during publish', 409);
    }
    async compileDraftProfileSnapshot(actor, profileID) {
    ...
    async freezeProfileResources(actor, profile) {
        const resources = await this.store.listAgentResources(actor.workspace_id);
        const freezeResources = async (refs, kind) => Promise.all(refs.map(async (ref) => {
            if (kind === 'mcp_server' && normalizedResourceID(ref.resource_id) === 'easydo') {
    ...
    async validateProfile(actor, profileID) {
        ...
        const resources = await this.store.listAgentResources(actor.workspace_id);
        const resolvePinnedResource = async (ref, kind) => {
            const resource = resources.find((candidate) => candidate.resource_kind === kind && refMatchesResource(ref, candidate));
            if (!resource)
                return { resource: undefined, version: undefined };
            try {
                return { resource, version: await this.resolveResourceVersion(actor, ref, resource, kind) };
            }
            catch (error) {
                if (error instanceof RuntimeDomainError)
                    return { resource, version: undefined, resolutionError: error };
                throw error;
            }
        };
        const recordFailure = (ref, code, message, path) => {
        ...
        for (const [index, ref] of profile.skills.entries()) {
            ...
            if (!version) {
                recordFailure(ref, resolutionError?.code || 'agent_profile_resource_version_not_found', resolutionError?.message || `Skill ${ref.resource_id} version was not found`, path);
                continue;
            }
        ...
        for (const [index, ref] of profile.mcp_servers.entries()) {
            ...
            if (!version) {
                recordFailure(ref, resolutionError?.code || 'agent_profile_resource_version_not_found', resolutionError?.message || `MCP server ${ref.resource_id} version was not found`, path);
                continue;
            }
            const spec = asRecord(version.snapshot.spec);
    ...
    async getVersion(actor, profileVersionID) {
    ...
    async getPublishedVersionByName(actor, profileName) {
        const profiles = await this.store.listProfiles(actor.workspace_id);
    ...
    async getLatestPublishedVersion(actor, profileID) {
        await this.getProfile(actor, profileID);
        const versions = (await this.store.listProfileVersions(actor.workspace_id, profileID))
            .filter((version) => version.status === 'published')
            .sort((left, right) => right.published_at.localeCompare(left.published_at) ||
    ...
    async ensurePageAssistantProfile(actor) {
    ...
    async materializePageAssistantProfile(actor) {
        const profiles = await this.store.listProfiles(actor.workspace_id);
        if (profiles.some((item) => item.name === PAGE_ASSISTANT_PROFILE_NAME))
            return;
        const profile = defaultPageAssistantProfile(await this.store.nextProfileId(), actor);
        assertProfileContextTags(profile.name, profile.context_tags);
        await this.store.saveProfile(profile);
    }
    async assertProfileNameUnique(workspaceID, profile) {
    ...
    async hasSubagentCycle(workspaceID, profileID) {
