...
export function resolveMcpClientConfigSecrets(config, secretRef = {}) {
    ...
    const resolveValue = (value) => {
        if (typeof value === 'string') {
            ...
            if (!resolved) {
                throw mcpClientConfigError('mcp_secret_ref_unresolved', `MCP secret_ref ${key} is not configured`);
            }
            return resolved;
        }
...
export function createMcpClient(config, logger = runtimeLogger) {
    ...
    if (type === 'sse') {
        ...
        if (!url)
            throw mcpClientConfigError('mcp_sse_url_required', 'MCP SSE URL is required');
        return new McpSseClient(url, headers, timeoutMs, logger);
    }
    if (type === 'stdio' || type === 'command') {
        const command = firstString(config.command);
        if (!command)
            throw mcpClientConfigError('mcp_stdio_command_required', 'MCP stdio command is required');
        return new McpStdioClient(command, stringArray(config.args), stringRecord(config.env), firstString(config.cwd), timeoutMs, logger);
    }
    ...
    if (!url)
        throw mcpClientConfigError('mcp_streamable_http_url_required', 'MCP Streamable HTTP URL is required');
...
export class McpStreamableHttpClient {
    ...
    initializeJob;
    requestSequence = 0;
    constructor(url, headers = {}, timeoutMs = 30_000, logger = runtimeLogger) {
        this.url = url;
        this.headers = headers;
        this.timeoutMs = timeoutMs;
        this.logger = logger;
    }
    async request(method, params, requestID, correlation = {}) {
        const context = { ...correlation, component: 'mcp-client', operation: method };
    ...
    async send(method, params, requestID) {
        const response = await this.sendMessage(method, params, false, true, requestID);
        return response.result;
    }
    async sendMessage(method, params, notification, includeSession, explicitID) {
    ...
    requestHeaders(includeSession = true) {
        return {
...
export class McpSseClient {
    ...
    async postMessage(method, params, notification = false, explicitID) {
        await this.ensureConnected();
...
export class McpStdioClient {
    ...
    stdoutBuffer = '';
    stderrBuffer = '';
    constructor(command, args = [], env = {}, cwd = '', timeoutMs = 30_000, logger = runtimeLogger) {
    ...
    async writeMessage(method, params, notification = false, explicitID) {
        await this.ensureStarted();
...
function parseMcpResponse(body, contentType, requestID) {
...
function getSecretRefPath(secretRef, key) {
    if (!key)
        return '';
    let current = secretRef;
...
export function buildStdioSandboxEnv(configuredEnv = {}) {
    const allowlist = [
        'PATH',
        'HOME',
        'TMPDIR',
...
function mcpClientConfigError(code, message) {
