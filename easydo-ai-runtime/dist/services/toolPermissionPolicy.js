export function evaluateToolPermission(input) {
    ...
    if (capabilityDecision === 'deny')
        return result('deny', `profile.capabilities.${capability}`, 'profile', permissionKey, operationType);
    if (input.requiresConfirmation)
        return result('ask', 'tool.requires_confirmation', 'tool', permissionKey, operationType);
    if (policy.all_tools_require_confirmation === true)
        return result('ask', 'profile.all_tools_require_confirmation', 'profile', permissionKey, operationType);
    if (capabilityDecision)
        return result(capabilityDecision, `profile.capabilities.${capability}`, 'profile', permissionKey, operationType);
    const defaultDecision = normalizeDecision(policy.default_decision ?? policy.defaultDecision);
    if (defaultDecision) {
        return result(defaultDecision, firstString(policy.default_matched_rule, policy.defaultMatchedRule, 'profile.default_decision'), firstString(policy.default_scope, policy.defaultScope, 'profile'), permissionKey, operationType);
    }
    return result('ask', 'default.missing_tool_action', 'default', permissionKey, operationType);
}
export function toolPermissionRules(policy, scope) {
    const toolMap = asRecord(policy.tools);
    const inlineMap = Object.fromEntries(Object.entries(policy).filter(([key]) => ![
        'tools',
        'rules',
        'default_decision',
        'defaultDecision',
...
function ruleMatches(rule, input) {
    ...
    if (tools.length > 0 && !tools.includes('*') && !tools.includes(toolName))
        return false;
    ...
    if (servers.length > 0 && !servers.includes('*') && !servers.includes(mcpServerKey))
        return false;
    const capabilities = stringList(rule.capabilities ?? rule.capability).map((item) => item.toLowerCase());
    if (capabilities.length > 0 && !capabilities.includes('*') && !capabilities.includes(capability))
        return false;
    ...
    if (operations.length > 0 && !operations.includes(operationType))
        return false;
    const roles = stringList(rule.roles ?? rule.actor_roles ?? rule.actor_role);
    if (roles.length > 0 && !roles.includes(firstString(input.actorRole)))
        return false;
...
function permissionCandidates(input) {
    ...
    if (server) {
        candidates.unshift(`mcp.${server}.${capability}.${input.toolName}`, `mcp.${server}.${capability}.${input.toolName}.${input.operationType}`, `mcp:${server}:${capability}:${input.toolName}:${input.operationType}`);
    }
    return candidates;
}
...
function result(decision, matchedRule, scope, permissionKey, operationType) {
    return {
        decision,
        matched_rule: matchedRule,
        scope,
