...
describe('classifyRuntimeError', () => {
    ...
    it('sanitizes unknown internal failures and never exposes secret-bearing input', () => {
        const classified = classifyRuntimeError(new Error('provider failed api_key=sk-sensitive password=hunter2 https://example.test?token=secret'), { source: 'runtime' });
    ...
    it('keeps an explicit stable code while applying canonical retry semantics', () => {
        expect(classifyRuntimeError(Object.assign(new Error('503 service unavailable'), { code: 'openrouter_unavailable', status: 503 }), { source: 'provider' })).toMatchObject({
    ...
    it('redacts secret-bearing JSON, headers, and Bearer credentials', () => {
        const classified = classifyRuntimeError(new Error('request failed body={"api_key":"json-api-secret","password":"json-password-secret"} '
    ...
    it('redacts provider credential aliases accepted by Runtime configuration', () => {
        const classified = classifyRuntimeError(new Error('access_token=access-secret bearer_token=bearer-secret refresh_token=refresh-secret '
    ...
    it('does not misattribute a generic Runtime timeout to the model provider', () => {
        expect(classifyRuntimeError(Object.assign(new Error('Runtime operation timed out'), { code: 'runtime_timeout' }), { source: 'runtime' })).toMatchObject({
            source: 'runtime',
