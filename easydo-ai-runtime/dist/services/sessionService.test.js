...
import { ActionService, SessionService } from './sessionService.js';
...
function stableJSONStringifyForTest(value) {
...
function leakingProviderFailureClient(secret) {
    return {
        async complete() {
            return { text: 'safe generated title' };
        },
        async *stream() {
            throw new ModelProviderError('openrouter_unavailable', `503 service unavailable api_key=${secret} token=provider-bearer-secret`, 503, {
...
class FakeAgentEventStore {
    ...
    async append(event) {
        ...
        if (asRecord(event).type === 'run.started' && explicitRunID)
            this.activeRunBySession.set(event.session_id, explicitRunID);
...
function firstStringForTest(...values) {
    return values.find((value) => typeof value === 'string' && value.trim());
}
function fakePiRunner(store, text = 'pi answer') {
    return {
        async prompt(input) {
...
function failingPiRunnerAfterStart(store, failure = 'Pi harness requires model.provider_id and model.id') {
    let promptCount = 0;
    return {
        async prompt(input) {
            promptCount += 1;
...
function allowToolPolicy(...toolNames) {
    return {
...
function chatboxSessionPayload(profileID, versionID = 'draft') {
    return {
        session_kind: 'chat',
...
function automaticTitleSessionPayload(profileID, versionID = 'draft') {
    const payload = chatboxSessionPayload(profileID, versionID);
    delete payload.title;
    return payload;
}
function pageAssistantPayload(overrides = {}) {
...
async function withMcpServer(handler) {
...
describe('SessionService Agent Profile Chatbox', () => {
    ...
    it('preserves Pi session runtime events in listed assistant entries', async () => {
        ...
        const listedRuntimeEvents = listedAssistant?.output.runtime_events;
    ...
    it('keeps token-level Pi deltas out of embedded entry and run runtime event summaries', async () => {
        ...
        const returnedRunEventTypes = (result.run?.result.runtime_events).map((event) => event.type);
        ...
        const listedAssistant = (await service.listEntries(actor, session.id))
            .find((entry) => entry.id === result.assistant_entry.id);
        const listedEventTypes = (listedAssistant?.output.runtime_events || []).map((event) => event.type);
        ...
        const replayEventTypes = replay.events.map((event) => event.event_type);
    ...
    it('replays Pi runtime events through the run event API shape', async () => {
        ...
        const textEvent = replay.events.find((event) => event.event_type === 'session.text.ended');
        ...
        expect(after.events.map((event) => event.event_type)).toEqual(['session.step.ended', 'run.completed']);
    });
    it('keeps historical Pi replay isolated after a second run in the same session', async () => {
        ...
        const service = new SessionService(store, profiles, chatClient('legacy'), undefined, {}, undefined, {
            runner: {
                async prompt(input) {
                    promptCount += 1;
                    const runtimeRunID = String(input.runtimeRunID);
                    const userMessageID = `pi-user-isolated-${promptCount}`;
                    const assistantMessageID = `pi-assistant-isolated-${promptCount}`;
                    await eventStore.append({ type: 'session.prompted', session_id: input.sessionID, runtime_run_id: runtimeRunID, event_id: '', seq: 0, timestamp: nowForTest(), message_id: userMessageID, prompt: input.prompt, delivery: 'prompt' });
                    await eventStore.append({ type: 'session.step.started', session_id: input.sessionID, runtime_run_id: runtimeRunID, event_id: '', seq: 0, timestamp: nowForTest(), assistant_message_id: assistantMessageID, parent_message_id: userMessageID });
                    await eventStore.append({ type: 'session.text.ended', session_id: input.sessionID, runtime_run_id: runtimeRunID, event_id: '', seq: 0, timestamp: nowForTest(), assistant_message_id: assistantMessageID, text_id: `pi-text-isolated-${promptCount}`, text: `answer ${promptCount}` });
                    await eventStore.append({ type: 'session.step.ended', session_id: input.sessionID, runtime_run_id: runtimeRunID, event_id: '', seq: 0, timestamp: nowForTest(), assistant_message_id: assistantMessageID, finish_reason: 'stop' });
                    return { session_id: input.sessionID, user_message_id: userMessageID, assistant_message_id: assistantMessageID };
                }
            },
            eventStore
        ...
        expect(firstReplay.events.map((event) => event.payload_json.runtime_run_id)).toEqual(expect.arrayContaining([first.run?.runtime_run_id]));
        expect(firstReplay.events.every((event) => event.payload_json.runtime_run_id === first.run?.runtime_run_id)).toBe(true);
        expect(JSON.stringify(firstReplay.events)).not.toContain('second isolated prompt');
        expect(secondReplay.events.every((event) => event.payload_json.runtime_run_id === second.run?.runtime_run_id)).toBe(true);
    ...
    it('finalizes Pi runs as failed when the harness fails after emitting early runtime events', async () => {
        ...
        const runtimeEvents = first.run?.result.runtime_events;
        ...
        expect(replay.events.find((event) => event.event_type === 'session.error')).toMatchObject({
        ...
        const secondRuntimeEvents = second.run?.result.runtime_events;
        ...
        expect(String(secondRuntimeEvents.find((event) => event.type === 'session.error')?.event_id)).toMatch(new RegExp(`^s_w${actor.workspace_id.toString(36)}_${session.id.toString(36).padStart(6, '0')}:\\d+$`));
    });
    it('classifies Pi provider timeouts as a timeout Run with one explicit timeout event', async () => {
        ...
        expect(replay.events.filter((event) => event.event_type === 'run.timeout')).toHaveLength(1);
    ...
    it('marks transient 429/rate-limit errors as retryable in session.error events', async () => {
        ...
        const runtimeEvents = result.run?.result.runtime_events;
    ...
    it('passes resolved provider, model, credential, and inference config into the Pi harness runner', async () => {
        ...
        expect((result.run?.result.runtime_events).map((event) => event.type)).toContain('model.selected');
    });
    it('keeps resolved provider credentials ephemeral while hashing the exact persisted Run snapshot', async () => {
        ...
        if (!result.run.profile_snapshot)
            throw new Error('run profile snapshot is required');
    ...
    it('exposes selected skills, MCP tools, and subagent descriptors progressively in the Pi harness runner', async () => {
        ...
        expect(capturedInput?.tools?.map((tool) => tool.name)).toEqual(['easydo_resource_list', 'subagent.spawn', 'easydo_skill_load']);
    ...
    it('runs a published profile with the resource content frozen at publish time', async () => {
        ...
        const frozenResources = published.snapshot.frozen_resources;
    ...
    it('does not load skills or execute MCP tools when the user only asks about pipelines', async () => {
        ...
        const eventTypes = (result.run?.result.runtime_events).map((event) => event.type);
    ...
    it('wires Pi subagent.spawn to real child run orchestration and stores parent-visible child links', async () => {
        ...
        const service = new SessionService(store, profiles, childModelClient, undefined, {}, undefined, {
            runner: {
                async prompt(input) {
                    const subagentTool = input.tools?.find((tool) => tool.name === 'subagent.spawn');
    ...
    it('wires Pi MCP tools to the runtime tool executor and stores tool lifecycle events', async () => {
        ...
        const runtimeSessionID = result.run?.request.runtime_session_id;
    ...
    it('keeps duplicate MCP tool approvals scoped by server key and capability', async () => {
        ...
        const approval = result.run?.result.awaiting_approval;
    ...
    it('persists MCP discovery timeouts before the Pi run continues', async () => {
        ...
        try {
            ...
            const replay = await service.listRunEventsByRuntimeID(String(result.run?.runtime_run_id));
            const timeoutEvent = replay.events.find((event) => event.event_type === 'mcp.tools_discovery_failed');
            ...
            const listedAssistant = (await service.listEntries(actor, session.id))
                .find((entry) => entry.id === result.assistant_entry.id);
            const listedTimeoutEvent = (listedAssistant?.output.runtime_events || [])
    ...
    it('marks Pi MCP tool runs as awaiting_decision when approval is required', async () => {
        ...
        const replay = await service.listRunEventsByRuntimeID(String(result.run?.runtime_run_id));
        expect(replay.events.map((event) => event.event_type)).toEqual(expect.arrayContaining(['permission.asked', 'run.awaiting_decision']));
        const approvalEvent = replay.events.find((event) => event.event_type === 'permission.asked');
    ...
    it('marks Pi MCP tool runs as awaiting_decision when Pi reports approval as a tool failure', async () => {
        ...
        const service = new SessionService(store, profiles, chatClient('legacy'), undefined, {}, undefined, {
            runner: {
                async prompt(input) {
                    ...
                    await eventStore.append({ type: 'session.tool.called', session_id: input.sessionID, event_id: '', seq: 0, timestamp: nowForTest(), assistant_message_id: assistantMessageID, call_id: 'call-pi-swallowed-approval-1', tool: 'easydo_pipeline_run_list', tool_name: 'easydo_pipeline_run_list', input: { workspace_id: 1 } });
                    await eventStore.append({ type: 'permission.asked', session_id: input.sessionID, event_id: '', seq: 0, timestamp: nowForTest(), request_id: 'approval:call-pi-swallowed-approval-1', approval_id: 'approval:call-pi-swallowed-approval-1', call_id: 'call-pi-swallowed-approval-1', tool_name: 'easydo_pipeline_run_list', reason: 'Tool easydo_pipeline_run_list requires approval', input: { workspace_id: 1 }, message: 'Tool easydo_pipeline_run_list requires approval' });
        ...
        expect(persistedEvents.map((event) => event.type)).toEqual(expect.arrayContaining([
            'session.prompted',
            'session.step.started',
            'session.tool.called',
            'permission.asked'
        ]));
        expect(persistedEvents.map((event) => event.type)).not.toEqual(expect.arrayContaining([
            'session.tool.failed',
            'session.text.ended',
            'session.step.ended'
        ]));
        ...
        const replay = await service.listRunEventsByRuntimeID(String(result.run?.runtime_run_id));
        const replayEventTypes = replay.events.map((event) => event.event_type);
        expect(replayEventTypes).toEqual(expect.arrayContaining(persistedEvents.map((event) => event.type)));
        ...
        const service = new SessionService(store, profiles, chatClient('legacy'), undefined, {}, undefined, {
            runner: {
                async prompt(input) {
                    ...
                    await eventStore.append({ type: 'session.tool.called', session_id: input.sessionID, event_id: '', seq: 0, timestamp: nowForTest(), assistant_message_id: assistantMessageID, call_id: 'call-pi-approval-narration-1', tool: 'easydo_resource_base_info_refresh', tool_name: 'easydo_resource_base_info_refresh', input: { resource_id: 2 } });
                    ...
                    throw new PiToolApprovalRequiredError('approval:call-pi-approval-narration-1', 'call-pi-approval-narration-1', 'easydo_resource_base_info_refresh', 'Tool easydo_resource_base_info_refresh requires approval');
                }
            },
            eventStore
        });
        ...
        const service = new SessionService(store, profiles, chatClient('legacy'), undefined, {}, undefined, {
            runner: {
                async prompt(input) {
                    ...
                    await eventStore.append({ type: 'session.prompted', session_id: input.sessionID, event_id: '', seq: 0, timestamp: nowForTest(), message_id: userMessageID, prompt: input.prompt, files: [], delivery: 'prompt' });
                    await eventStore.append({ type: 'session.step.started', session_id: input.sessionID, event_id: '', seq: 0, timestamp: nowForTest(), assistant_message_id: assistantMessageID, parent_message_id: userMessageID, agent: input.agent, model: input.model });
                    await eventStore.append({ type: 'session.tool.called', session_id: input.sessionID, event_id: '', seq: 0, timestamp: nowForTest(), assistant_message_id: assistantMessageID, provider_tool_call_id: 'call-provider-only-1', tool: 'easydo_pipeline_run_list', tool_name: 'easydo_pipeline_run_list', input: { workspace_id: 1 } });
                    await eventStore.append({ type: 'permission.asked', session_id: input.sessionID, event_id: '', seq: 0, timestamp: nowForTest(), request_id: 'approval:call-provider-only-1', approval_id: 'approval:call-provider-only-1', provider_tool_call_id: 'call-provider-only-1', tool_name: 'easydo_pipeline_run_list', reason: 'Tool easydo_pipeline_run_list requires approval', input: { workspace_id: 1 }, message: 'Tool easydo_pipeline_run_list requires approval' });
                    await eventStore.append({ type: 'session.tool.failed', session_id: input.sessionID, event_id: '', seq: 0, timestamp: nowForTest(), assistant_message_id: assistantMessageID, provider_tool_call_id: 'call-provider-only-1', error: { type: 'unknown', message: 'Tool easydo_pipeline_run_list requires approval' } });
    ...
    it('continues Pi MCP tool runs after approval and completes the runtime run', async () => {
        ...
        expect(continued.run.result.runtime_events.map((event) => event.type)).toEqual(expect.arrayContaining([
            'permission.resolved',
            'session.tool.called',
        ...
        expect(replay.events.map((event) => event.event_type)).toEqual(expect.arrayContaining([
            'permission.asked',
            'permission.resolved',
            'session.tool.called',
            'session.tool.success',
        ...
        const service = new SessionService(store, profiles, chatClient('legacy'), {
            ...
            runner: {
                async prompt(input) {
                    ...
                    if (prompts.length === 1)
                        await input.tools?.[0].execute('call-steer-1', { target_id: 'resource-1' });
        ...
        const service = new SessionService(store, profiles, chatClient('legacy'), undefined, { autoDrainSessionQueue: false }, undefined, {
            runner: {
                async prompt(input) {
                    ...
                    if (!currentService)
                        throw new Error('expected session service');
    ...
    it('keeps Pi runs awaiting when one approval from a multi-tool batch is still unresolved', async () => {
            ...
            runner: {
                async prompt(input) {
                    ...
                    for (const callID of ['call-pi-multi-1', 'call-pi-multi-2']) {
                        await eventStore.append({ type: 'session.tool.called', session_id: input.sessionID, event_id: '', seq: 0, timestamp: nowForTest(), assistant_message_id: assistantMessageID, call_id: callID, tool: 'easydo_pipeline_run_list', tool_name: 'easydo_pipeline_run_list', input: { pipeline_id: callID.endsWith('1') ? 14 : 13 } });
                        await eventStore.append({ type: 'permission.asked', session_id: input.sessionID, event_id: '', seq: 0, timestamp: nowForTest(), request_id: `approval:${callID}`, approval_id: `approval:${callID}`, call_id: callID, tool_name: 'easydo_pipeline_run_list', reason: 'Tool easydo_pipeline_run_list requires approval', input: { pipeline_id: callID.endsWith('1') ? 14 : 13 }, message: 'Tool easydo_pipeline_run_list requires approval' });
        ...
        const listedRuntimeEventTypes = (listedAssistant?.output.runtime_events).map((event) => event.type);
            ...
            runner: {
                async prompt(input) {
                    ...
                    for (const callID of ['call-pi-corrupt-1', 'call-pi-corrupt-2']) {
                        await eventStore.append({ type: 'session.tool.called', session_id: input.sessionID, event_id: '', seq: 0, timestamp: nowForTest(), assistant_message_id: assistantMessageID, call_id: callID, tool: 'easydo_pipeline_run_list', tool_name: 'easydo_pipeline_run_list', input: { pipeline_id: callID.endsWith('1') ? 1 : 13 } });
                        await eventStore.append({ type: 'permission.asked', session_id: input.sessionID, event_id: '', seq: 0, timestamp: nowForTest(), request_id: `approval:${callID}`, approval_id: `approval:${callID}`, call_id: callID, tool_name: 'easydo_pipeline_run_list', reason: 'Tool easydo_pipeline_run_list requires approval', input: { pipeline_id: callID.endsWith('1') ? 1 : 13 }, message: 'Tool easydo_pipeline_run_list requires approval' });
                    }
    ...
    it('rejects stale Pi approvals after a previous approval already let the agent loop continue', async () => {
        ...
        await eventStore.append({ type: 'permission.resolved', session_id: runtimeSessionID, event_id: '', seq: 0, timestamp: nowForTest(), request_id: 'approval:call-pi-stale-2', approval_id: 'approval:call-pi-stale-2', call_id: 'call-pi-stale-2', tool_name: 'easydo_pipeline_run_list', result: 'approved', decision: 'approve_once' });
        await eventStore.append({ type: 'session.tool.called', session_id: runtimeSessionID, event_id: '', seq: 0, timestamp: nowForTest(), call_id: 'call-pi-stale-2', tool: 'easydo_pipeline_run_list', tool_name: 'easydo_pipeline_run_list', input: { pipeline_id: 13 } });
        await eventStore.append({ type: 'session.tool.success', session_id: runtimeSessionID, event_id: '', seq: 0, timestamp: nowForTest(), call_id: 'call-pi-stale-2', tool: 'easydo_pipeline_run_list', tool_name: 'easydo_pipeline_run_list', result: { content: [{ type: 'text', text: '{"ok":true}' }] } });
        await eventStore.append({ type: 'session.step.started', session_id: runtimeSessionID, event_id: '', seq: 0, timestamp: nowForTest(), assistant_message_id: 'pi-assistant-stale-approval-2', model: { provider_id: 'test', id: 'fake' } });
        await store.saveRun({
            ...firstRun,
            status: 'completed',
            finished_at: nowForTest(),
    ...
    it('streams Pi approval continuation events while completing the runtime run', async () => {
        ...
        const finalAssistant = streamed.find((event) => event.event === 'assistant_entry')?.data.entry;
    ...
    it('records OpenCode-style cancellation events when cancelling a Pi run awaiting approval', async () => {
        ...
        const runtimeEvents = cancelled.run.result.runtime_events;
    ...
    it('replays cancellation events for early Pi runs whose result has not recorded the runtime engine yet', async () => {
        ...
        if (!stored)
            throw new Error('stored run missing');
        await store.saveRun({
            ...stored,
            result: {},
    ...
    it('replays Pi compaction lifecycle events from completed runtime runs', async () => {
        ...
        const replay = await service.listRunEventsByRuntimeID(String(result.run?.runtime_run_id));
        const compactionEvents = replay.events.filter((event) => event.event_type.startsWith('session.compaction.'));
        expect(compactionEvents.map((event) => event.event_type)).toEqual([
    ...
    it('repairs an expired Pi owner lease as one interrupted terminal Run', async () => {
        ...
        expect(replay.events.filter((event) => event.event_type === 'run.interrupted')).toHaveLength(1);
    ...
    it('streams runtime_engine=pi entries through the AgentHarnessRunner event core', async () => {
        ...
        expect((assistant?.output?.runtime_events).map((event) => event.type)).toEqual([
            'run.started',
            'context.build_started',
            'capability.snapshot',
            'context.build_completed',
    ...
    it('treats a bare object output schema as unconstrained chat output', async () => {
        ...
        const replay = await service.listRunEventsByRuntimeID(String(result.run?.runtime_run_id));
        expect(replay.events.filter((event) => event.event_type === 'run.completed')).toHaveLength(1);
        expect(replay.events.at(-1)).toMatchObject({
    ...
    it('persists one explicit failed state event for legacy model failures', async () => {
        ...
        const replay = await service.listRunEventsByRuntimeID(String(result.run?.runtime_run_id));
        expect(result.run?.status).toBe('failed');
        expect(replay.events.filter((event) => event.event_type === 'run.failed')).toHaveLength(1);
        expect(replay.events.at(-1)).toMatchObject({
    ...
    it('stores provider raw responses as internal artifacts instead of normal output payload', async () => {
        ...
        const rawRefs = result.run?.result.provider_raw_refs;
    ...
    it('truncates oversized tool output into preview plus artifact and tells the model', async () => {
        ...
        const toolResults = result.run?.result.tool_results;
    ...
    it('emits runtime approval request schema and permission events for gated tool calls', async () => {
        ...
        const action = (result.run?.result.agent_actions)[0];
    ...
    it('continues streaming runs when the provider first returns reasoning and blank text', async () => {
        ...
        const assistantEntry = assistantEvent?.data?.entry;
    ...
    it('persists streaming reasoning and answer deltas as runtime process events', async () => {
        ...
        expect(runtimeEvents.findIndex((event) => event.type === 'reasoning_delta')).toBeLessThan(runtimeEvents.findIndex((event) => event.type === 'answer_delta'));
    });
    ...
    it('persists a safe provider error descriptor when bounded continuation also fails', async () => {
        ...
        const assistantEvent = events.find((event) => event.event === 'assistant_entry');
        const assistantEntry = assistantEvent?.data?.entry;
        const providerError = assistantEntry?.output?.provider_error;
    ...
    it('assembles multiple context tag fragments in profile order before the user message', async () => {
        ...
        const client = {
            async complete(request) {
                ...
                capturedCapabilities = request.capabilities;
    ...
    it('executes a confirmed tool call and continues the final model answer', async () => {
        ...
        const agentActions = firstRun.assistant_entry.output.agent_actions;
    ...
    it('rejects an action continuation before decisions, state changes, entries, or tool execution when the Run snapshot hash is tampered', async () => {
        ...
        const run = first.run;
        if (!run)
            throw new Error('runtime run must exist');
        const actionID = String(first.assistant_entry.output.agent_actions[0]?.id);
        const actionBefore = await store.getActionByBusinessID(actor.workspace_id, actionID);
        if (!actionBefore)
            throw new Error('pending action must exist');
        const entriesBefore = await store.listEntries(actor.workspace_id, session.id);
    ...
    it('lists chat entries with lightweight runtime event summaries instead of full delta snapshots', async () => {
        ...
        if (!storedAssistant)
            throw new Error('stored assistant entry must exist');
        const storedEvents = storedAssistant.output.runtime_events;
        const listedEntries = await service.listEntries(actor, session.id);
        const listedAssistant = listedEntries.find((entry) => entry.id === storedAssistant.id);
        if (!listedAssistant)
            throw new Error('assistant entry must be listed');
        ...
        expect(listedAssistant.output.runtime_events.some((event) => event.type === 'reasoning_delta')).toBe(false);
        expect(listedAssistant.output.runtime_events.some((event) => event.type === 'answer_delta')).toBe(false);
    });
    it('does not expose internal runtime event sequence conflicts through entry or Pi run event projections', async () => {
        ...
        expect(replay.events.map((event) => event.event_seq)).toEqual([...visibleEventSeqs, 13]);
        expect(scopedReplay.events.map((event) => event.event_seq)).toEqual([...visibleEventSeqs, 13]);
        ...
        const replayNarration = replay.events.flatMap((event) => {
            const payload = event.payload_json || {};
            return [payload.text, payload.delta, payload.message].filter((value) => typeof value === 'string');
        }).join('');
    ...
    it('runs subagents through their own Pi loop with child-scoped tools and events', async () => {
        ...
        const spawned = (result.run?.result.runtime_events)
        ...
        expect(childReplay.events.map((event) => event.event_type)).toEqual(expect.arrayContaining([
            'run.started',
            'session.tool.called',
            'session.tool.success',
            'run.completed'
    ...
    it('keeps child Pi runs awaiting approval and resumes the child without failing the parent thread', async () => {
        ...
        const parentEvents = parentResult.run?.result.runtime_events;
        ...
        expect(executed).toEqual([]);
        if (!childBefore)
            throw new Error('child run missing before approval');
        ...
        if (!readOnlyChild)
            throw new Error('read-only child run missing');
    ...
    it('streams parent-visible subagent lifecycle before the parent model call', async () => {
        ...
        const profiles = new AgentProfileService(store);
        let releaseChild = () => { };
        const childCanFinish = new Promise((resolve) => {
        ...
        const nextEventWithin = async (timeoutMs) => {
            return Promise.race([
                iterator.next(),
                new Promise((resolve) => {
        ...
        let spawnedBeforeChildCompletion = null;
        try {
    ...
    it('does not stream internal action continuation plan text before falling back to the tool result summary', async () => {
        const store = createMemoryRuntimeStore();
        const profiles = new AgentProfileService(store);
        const completeRefreshRun = async (request) => {
            if (request.content.includes('action_result')) {
        ...
        const answerDeltas = events
            ...
            .join('');
        const assistantEntry = events.find((event) => event.event === 'assistant_entry')?.data.entry;
    ...
    it('uses approve_session grants to skip repeated approval in the same session and resource scope', async () => {
        ...
        const firstAction = firstRun.assistant_entry.output.agent_actions[0];
        expect(firstRun.run?.status).toBe('awaiting_decision');
        ...
        const continuationEvents = continued.assistant_entry.output.runtime_events;
        ...
        const secondActions = secondRun.assistant_entry.output.agent_actions;
    ...
    it('persists approved actions and user decision entries as first-class session history', async () => {
        ...
        expect(executeCount).toBe(1);
        expect(continued.action.status).toBe('executed');
        expect(entries.some((entry) => entry.role === 'user' &&
            entry.entry_type === 'run_event' &&
            entry.content.includes('已确认') &&
            String(entry.input.action_id || '') === actionID)).toBe(true);
    });
    ...
    it('deduplicates semantically identical pending tool actions in one run', async () => {
        ...
        const actions = result.assistant_entry.output.agent_actions;
    ...
    it('hydrates action status when listing existing entries', async () => {
        ...
        const hydrated = originalAssistant?.output.agent_actions;
    ...
    it('applies server-scoped MCP permission rules to actual model tool calls', async () => {
        ...
        const agentActions = result.assistant_entry.output.agent_actions;
        expect(result.run?.status).toBe('completed');
    ...
    it('automatically executes read-only JSON tool calls in stream runs and continues with the tool result', async () => {
        ...
        const eventsBeforeToolCompletion = [];
        for await (const event of service.streamEntryRun(actor, session.id, {
            content: '查询 GPU 使用率',
        ...
        const agentActions = assistantEntry?.output?.agent_actions;
        expect(agentActions?.[0]).toMatchObject({
