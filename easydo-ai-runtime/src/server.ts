...
import { runtimeEnvelopeSchema, type RuntimeEnvelope } from './schemas/runtime.js'
import { AgentProfileService, RuntimeDomainError, probeMcpServerConfig } from './services/profileService.js'
import { ActionService, SessionService, type AgentRuntimeEngineOptions, type RuntimeStreamEvent } from './services/sessionService.js'
import type { ChatModelClient } from './services/modelProvider.js'
import { createMariadbRuntimeStore } from './store/mariadbRuntimeStore.js'
import { createMemoryRuntimeStore, type RuntimeStore } from './store/memoryRuntimeStore.js'
import { InMemoryAgentEventStore, type AgentEventStore } from './agent-runtime/eventStore.js'
...
export interface CreateAppOptions {
  ...
  agentHarnessRunner?: AgentRuntimeEngineOptions['runner']
  ...
  workspaceRuntime?: NonNullable<AgentRuntimeEngineOptions['workspace']>
  agentWorkspaceService?: AgentWorkspaceService
  logger?: RuntimeLogger
}

type RunOwnerReconcile = (force?: boolean) => Promise<unknown>

export function startRunOwnerSweeper(
  reconcile: RunOwnerReconcile,
  intervalMs: number,
  onError?: (error: unknown) => void,
  logger: RuntimeLogger = runtimeLogger
) {
...
type RuntimeApprovalDecision = 'approve_once' | 'approve_session' | 'reject' | 'steer'

function parseActionDecision(payload: Record<string, unknown>): RuntimeApprovalDecision {
  const raw = String(payload.decision || payload.action || '').trim().toLowerCase()
  if (raw === 'approve_once' || raw === 'approve_session' || raw === 'steer') return raw
...
async function readEnvelope(c: Context): Promise<RuntimeEnvelope> {
  const body = await c.req.json()
  return runtimeEnvelopeSchema.parse(body)
}

function runtimePayload(envelope: RuntimeEnvelope) {
  return { ...envelope.payload, runtime_request_id: envelope.request_id }
}

function errorBody(error: unknown, requestID = '', runtimeRunID = '') {
...
export async function createSseResponse(
  factory: () => AsyncIterable<RuntimeStreamEvent>,
  requestID = '',
  options: { noTransform?: boolean, metrics?: RuntimeMetrics, operation?: string, logger?: RuntimeLogger } = {}
) {
  ...
  let first: IteratorResult<RuntimeStreamEvent>
  try {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const enqueue = (item: RuntimeStreamEvent) => {
      ...
      try {
        ...
        if (outcome === 'failed') logger.error(logInput)
        else logger.info(logInput)
        controller.close()
      }
    },
    async cancel() {
...
function isRuntimeFirstResponseEvent(item: RuntimeStreamEvent) {
...
function runtimeLatencyWindow(metrics: RuntimeMetrics, windowMs: number, observedAt: number) {
  return {
...
function createAppContext(options: CreateAppOptions) {
  ...
  const sessionService = new SessionService(
    ...
    undefined,
    { runner: agentHarnessRunner, eventStore: agentEventStore, workspace: options.workspaceRuntime }
  )
  const actionService = new ActionService(store, sessionService)
  const agentWorkspaceService = options.agentWorkspaceService
  const requestLogContexts = new WeakMap<Request, RuntimeLogInput>()
  ...
  app.get('/readyz', async (c) => {
    const persistentStore = store as typeof store & { ping?: () => Promise<void> }
  ...
  app.post('/v1/sessions/:id/queue/items', async (c) => {
    const envelope = await readEnvelope(c)
    return c.json(ok(await sessionService.enqueueSessionQueueItem(
      envelope.actor,
      parseID(c.req.param('id'), 'session_id'),
      envelope.payload
    )))
  })
  ...
  app.post('/v1/sessions/:id/queue/query', async (c) => {
    const envelope = await readEnvelope(c)
    return c.json(ok(await sessionService.listSessionQueueItems(
      envelope.actor,
      parseID(c.req.param('id'), 'session_id')
    )))
  })
  ...
  app.post('/v1/sessions/:id/queue/items/:queue_item_id/cancel', async (c) => {
    const envelope = await readEnvelope(c)
    return c.json(ok(await sessionService.cancelSessionQueueItem(
      envelope.actor,
      parseID(c.req.param('id'), 'session_id'),
      String(c.req.param('queue_item_id') || '')
    )))
  })

  app.post('/v1/sessions/:id/queue/reorder', async (c) => {
    const envelope = await readEnvelope(c)
    return c.json(ok(await sessionService.reorderSessionQueueItems(
      envelope.actor,
      parseID(c.req.param('id'), 'session_id'),
      envelope.payload
    )))
  ...
  app.post('/v1/sessions/:id/entries/stream', async (c) => {
    ...
    return createSseResponse(
      () => sessionService.streamEntryRun(envelope.actor, sessionID, payload, envelope.auth),
      envelope.request_id,
      { metrics, operation: 'entry_stream', logger }
    )
  })
  ...
  app.post('/v1/actions/:id/decision/stream', async (c) => {
    ...
    const decision = parseActionDecision(envelope.payload)
    return createSseResponse(
      () => actionService.decideAndContinueStream(envelope.actor, String(c.req.param('id') || ''), decision, envelope.payload, envelope.auth),
      envelope.request_id,
      { metrics, operation: 'action_decision_stream', logger }
    )
  })

  app.post('/v1/runs/:runtime_run_id/pi-approval/decision', async (c) => {
    const envelope = await readEnvelope(c)
    return c.json(ok(await sessionService.decidePiApproval(
      envelope.actor,
      String(c.req.param('runtime_run_id') || ''),
      envelope.payload,
      envelope.auth
    )))
  })
  ...
  app.post('/v1/runs/:runtime_run_id/pi-approval/decision/stream', async (c) => {
    ...
    return createSseResponse(
      () => sessionService.streamPiApproval(envelope.actor, runtimeRunID, envelope.payload, envelope.auth),
      envelope.request_id,
      { metrics, operation: 'pi_approval_stream', logger }
    )
  })
  ...
  app.post('/v1/runs/:runtime_run_id/events/stream', async (c) => {
    ...
    return createSseResponse(
      () => sessionService.streamRunEvents(envelope.actor, runtimeRunID, envelope.payload),
      envelope.request_id,
      { noTransform: true, metrics, operation: 'run_event_stream', logger }
    )
  })
  ...
  app.post('/v1/runtime-events', async (c) => {
    const envelope = await readEnvelope(c)
    const event = envelope.payload.event as AgentRuntimeEvent | undefined
  ...
  app.post('/v1/runtime-events/query', async (c) => {
    ...
    if (!sessionID) throw new RuntimeDomainError('session_id_required', 'session_id is required')
  ...
  app.post('/v1/runtime-harness/prompt', async (c) => {
    ...
    if (!sessionID) throw new RuntimeDomainError('session_id_required', 'session_id is required')
    if (!runtimeRunID) throw new RuntimeDomainError('runtime_run_id_required', 'runtime_run_id is required')
    if (!prompt) throw new RuntimeDomainError('prompt_required', 'prompt is required')
    const result = await agentHarnessRunner.prompt({
      ...
      model: typeof envelope.payload.model === 'object' && envelope.payload.model !== null
        ? envelope.payload.model as { provider_id: string; id: string; variant?: string }
        : undefined
    })
...
export function start(config: RuntimeConfig = loadConfig()) {
  ...
  const sweeper = startRunOwnerSweeper(
    (force) => sessionService.reconcileExpiredRunOwners(force),
    config.runStreamPollMs
  )
  const server = serve({
    fetch: app.fetch,
    port: config.port
