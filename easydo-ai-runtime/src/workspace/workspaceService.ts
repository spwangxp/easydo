...
import type { AIRuntimeWorkspace, AIRuntimeWorkspaceAudit } from './types.js'
...
export class AgentWorkspaceService {
  ...
  private readonly defaultProviderEndpoint: string

  constructor(
    private readonly store: RuntimeStore,
    private readonly provider: AgentWorkspaceProvider,
    options: AgentWorkspaceServiceOptions = {}
  ) {
  ...
  private async provisionDefault(workspace: AIRuntimeWorkspace, actorUserID: number) {
    ...
    if (!claimed) return this.waitForDefaultProvision(workspace)
    ...
    try {
      ...
      if (!ready) return this.waitForDefaultProvision(claimed)
  ...
  private async waitForDefaultProvision(workspace: AIRuntimeWorkspace) {
    for (let attempt = 0; attempt < 100; attempt += 1) {
      const current = await this.store.getAgentWorkspace(workspace.workspace_id, workspace.workspace_runtime_id)
      if (!current) break
      if (current.status === 'ready' || current.status === 'paused') return current
  ...
  async get(actor: RuntimeActor, workspaceRuntimeID: string) {
  ...
  async pause(actor: RuntimeActor, workspaceRuntimeID: string) {
    const current = await this.get(actor, workspaceRuntimeID)
    if (current.status === 'paused') return current
    if (current.status !== 'ready') throw workspaceStateError(current, ['ready'])
    await this.provider.pause(current)
    const paused: AIRuntimeWorkspace = {
      ...current,
      status: 'paused',
  ...
  async resume(actor: RuntimeActor, workspaceRuntimeID: string) {
    const current = await this.get(actor, workspaceRuntimeID)
    if (current.status === 'ready') return current
    if (current.status !== 'paused') throw workspaceStateError(current, ['paused'])
    await this.provider.resume(current)
    const resumed: AIRuntimeWorkspace = {
      ...current,
      status: 'ready',
      paused_at: undefined,
      updated_at: this.now()
  ...
  async recycle(actor: RuntimeActor, workspaceRuntimeID: string) {
    const current = await this.get(actor, workspaceRuntimeID)
    ...
    const observedAt = this.now()
    const recycling = await this.store.beginAgentWorkspaceRecycle(
      current.workspace_id,
      current.workspace_runtime_id,
      current.owner_user_id,
      `workspace-recycle:${randomUUID()}`,
      new Date(Date.parse(observedAt) + 5 * 60_000).toISOString(),
      observedAt
    )
    if (!recycling) throw workspaceInUseError(current)
    ...
    let provisioned: { sandbox_id: string }
    try {
  ...
  private async failRecycle(
    workspace: AIRuntimeWorkspace,
    actorUserID: number,
    phase: 'recycle' | 'provision',
    errorCode: 'agent_workspace_recycle_failed' | 'agent_workspace_reprovision_failed',
    error: unknown,
    recycledAt?: string
  ): Promise<never> {
    const errorMessage = error instanceof Error ? error.message : String(error)
  ...
  private async requireStatus(actor: RuntimeActor, workspaceRuntimeID: string, statuses: AIRuntimeWorkspace['status'][]) {
    const workspace = await this.get(actor, workspaceRuntimeID)
    if (!statuses.includes(workspace.status)) throw workspaceStateError(workspace, statuses)
    return workspace
  }
...
function workspaceRuntimeIDFor(workspaceID: number, userID: number, key: string) {
  return `aws_w${workspaceID}_u${userID}_${key.replace(/[^a-zA-Z0-9_-]+/g, '-').replace(/^-+|-+$/g, '') || 'default'}`
}

function workspaceSnapshotHash(workspace: AIRuntimeWorkspace) {
...
function workspaceStateError(workspace: AIRuntimeWorkspace, expected: AIRuntimeWorkspace['status'][]) {
  return new RuntimeDomainError(
    'agent_workspace_not_ready',
    `Agent workspace ${workspace.workspace_runtime_id} is ${workspace.status}; expected ${expected.join(' or ')}`,
    409,
    { workspace_status: workspace.status, expected_statuses: expected }
  )
}

function workspaceInUseError(workspace: AIRuntimeWorkspace) {
  return new RuntimeDomainError(
    'agent_workspace_in_use',
    `Agent workspace ${workspace.workspace_runtime_id} is in use by an active run`,
    409
  )
}
