...
function workspace(): AIRuntimeWorkspace {
  return {
    id: 1,
...
function client() {
  ...
  const implementation: WorkspaceExecutionClient = {
    ...
    async fileInfo(_workspace, path) {
      ...
      return { name: 'demo.txt', path, kind: 'file' as const, size: 5, mtimeMs: 1 }
    },
    async listDir(_workspace, path) {
      calls.push({ method: 'listDir', args: [path] })
      return [{ name: 'demo.txt', path: `${path}/demo.txt`, kind: 'file' as const, size: 5, mtimeMs: 1 }]
    },
