...
import type { FileInfo, ShellExecOptions } from '@earendil-works/pi-agent-core'
import { z } from 'zod'
import type { AIRuntimeWorkspace } from './types.js'
import type { AgentWorkspaceProvider } from './workspaceProvider.js'
...
export class DockerWorkspaceProvider implements AgentWorkspaceProvider, WorkspaceExecutionClient {
  ...
  async provision(workspace: AIRuntimeWorkspace) {
    ...
    if (!isMissingContainer(inspected)) throw commandError(`inspect Agent workspace ${workspace.workspace_runtime_id}`, inspected)
    ...
    if (runResult.exitCode === 0) return { sandbox_id: sandboxID }
  ...
  async exec(workspace: AIRuntimeWorkspace, command: string, options: ShellExecOptions & { cwd: string }) {
  ...
  async readFile(workspace: AIRuntimeWorkspace, targetPath: string, abortSignal?: AbortSignal) {
  ...
  async writeFile(workspace: AIRuntimeWorkspace, targetPath: string, content: Uint8Array, append: boolean, abortSignal?: AbortSignal) {
  ...
  async fileInfo(workspace: AIRuntimeWorkspace, targetPath: string, abortSignal?: AbortSignal) {
    return await this.fileOperation(workspace, 'stat', { path: targetPath, root: workspace.root_path }, undefined, abortSignal) as unknown as FileInfo
  }

  async listDir(workspace: AIRuntimeWorkspace, targetPath: string, abortSignal?: AbortSignal) {
    const value = await this.fileOperation(workspace, 'list', { path: targetPath, root: workspace.root_path }, undefined, abortSignal)
    return Array.isArray(value.entries) ? value.entries as FileInfo[] : []
  }

  async canonicalPath(workspace: AIRuntimeWorkspace, targetPath: string, abortSignal?: AbortSignal) {
  ...
  async exists(workspace: AIRuntimeWorkspace, targetPath: string, abortSignal?: AbortSignal) {
    const value = await this.fileOperation(workspace, 'exists', { path: targetPath, root: workspace.root_path }, undefined, abortSignal)
    return value.exists === true
  }

  async createDir(workspace: AIRuntimeWorkspace, targetPath: string, recursive: boolean, abortSignal?: AbortSignal) {
    await this.fileOperation(workspace, 'mkdir', { path: targetPath, root: workspace.root_path, recursive }, undefined, abortSignal)
  }

  async remove(workspace: AIRuntimeWorkspace, targetPath: string, recursive: boolean, force: boolean, abortSignal?: AbortSignal) {
    await this.fileOperation(workspace, 'remove', { path: targetPath, root: workspace.root_path, recursive, force }, undefined, abortSignal)
  }

  async createTempDir(workspace: AIRuntimeWorkspace, prefix: string, abortSignal?: AbortSignal) {
  ...
  async createTempFile(workspace: AIRuntimeWorkspace, prefix: string, suffix: string, abortSignal?: AbortSignal) {
  ...
  private async fileOperation(
    ...
    if (result.exitCode !== 0) throw fileCommandError(result)
    try {
  ...
  private async mustRun(args: string[], action: string) {
    const result = await this.run(args)
    if (result.exitCode !== 0) throw commandError(action, result)
    return result
  }

  private run(args: string[], options?: DockerCommandOptions) {
    return this.runner.run(this.dockerHost ? ['--host', this.dockerHost, ...args] : args, options)
  }

  private volumeName(workspace: AIRuntimeWorkspace) {
  ...
  async run(args: string[], options: DockerCommandOptions = {}): Promise<DockerCommandResult> {
    return new Promise((resolve, reject) => {
      ...
      child.stdout.on('data', (chunk: Buffer) => {
        ...
        stdout += text
        try { options.onStdout?.(text) } catch (error) {
          child.kill('SIGKILL')
      ...
      child.stderr.on('data', (chunk: Buffer) => {
        const text = chunk.toString('utf8')
        stderr += text
        try { options.onStderr?.(text) } catch (error) {
          child.kill('SIGKILL')
          finish(() => reject(Object.assign(asError(error), { name: 'CallbackError' })))
        }
      })
...
function requiredSandboxID(workspace: AIRuntimeWorkspace) {
  const sandboxID = workspace.sandbox_id || `easydo-ai-${workspace.workspace_runtime_id}`
  if (!/^[a-zA-Z0-9][a-zA-Z0-9_.-]+$/.test(sandboxID)) throw new Error(`Invalid Docker sandbox ID: ${sandboxID}`)
  return sandboxID
}

function commandError(action: string, result: DockerCommandResult) {
  return new Error(`Failed to ${action}: ${(result.stderr || result.stdout || `exit ${result.exitCode}`).trim()}`)
}

function isMissingContainer(result: DockerCommandResult) {
...
function requireRunningWorkspaceContainer(result: DockerCommandResult, workspaceRuntimeID: string) {
...
function isRunningWorkspaceContainer(result: DockerCommandResult, workspaceRuntimeID: string) {
  let value: unknown
  try {
...
function isConcurrentProvisionWinner(result: DockerCommandResult, workspaceRuntimeID: string) {
...
function fileCommandError(result: DockerCommandResult) {
  try {
    const parsed = JSON.parse(result.stderr.trim()) as { code?: string, message?: string }
...
function requiredString(value: unknown, label: string) {
  if (typeof value !== 'string') throw new Error(`Docker workspace response is missing ${label}`)
  return value
}
