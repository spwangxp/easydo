...
describe('AI Agent workspace store', () => {
  it('isolates workspace records by EasyDo workspace and preserves the owner', async () => {
    const store = createMemoryRuntimeStore() as any
  ...
  it('claims one workspace lease atomically and increments the epoch after expiry', async () => {
    ...
    const first = await store.claimAgentWorkspaceLease(
      1,
      'aws_w1_u7_default',
      'runtime-a',
    ...
    await expect(store.claimAgentWorkspaceLease(
      1,
      'aws_w1_u7_default',
      'runtime-b',
    ...
    const takeover = await store.claimAgentWorkspaceLease(
      1,
      'aws_w1_u7_default',
      'runtime-b',
      '2026-07-12T00:00:21.000Z',
      '2026-07-12T00:00:11.000Z'
    )
  ...
  it.each(['ready', 'paused', 'failed'])('allows only one lifecycle recycle claim from %s', async (status) => {
    ...
    const first = await store.beginAgentWorkspaceRecycle(
      1,
      'aws_w1_u7_default',
      7,
      'recycle-operation-a',
