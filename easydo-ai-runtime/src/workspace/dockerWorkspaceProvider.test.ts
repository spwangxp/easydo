...
import { DockerWorkspaceProvider, type DockerCommandRunner } from './dockerWorkspaceProvider.js'
...
function runner() {
  const calls: string[][] = []
  const implementation: DockerCommandRunner = {
    async run(args) {
      calls.push(args)
      if (args.includes('inspect')) return { stdout: '', stderr: 'not found', exitCode: 1 }
      if (args.includes('run')) return { stdout: 'container-id\n', stderr: '', exitCode: 0 }
...
describe('DockerWorkspaceProvider', () => {
  ...
  it('accepts the running workspace created by a concurrent provisioner after a Docker name conflict', async () => {
    ...
    const provider = new DockerWorkspaceProvider({
      async run(args) {
        ...
        if (args[0] === 'inspect') {
          inspectCount += 1
          if (inspectCount === 1) return { stdout: '', stderr: 'Error: No such container: easydo-ai-w1-u7-default', exitCode: 1 }
          return {
  ...
  it('fails recycle when the persistent workspace volume cannot be removed', async () => {
    const provider = new DockerWorkspaceProvider({
      async run(args) {
        if (args[0] === 'volume') return { stdout: '', stderr: 'volume is in use', exitCode: 1 }
        return { stdout: '', stderr: '', exitCode: 0 }
      }
    })
