...
import type { AgentResource } from './runtime.js'
...
describe('AgentResourceVersion', () => {
  it('creates a stable sha256 digest from an unresolved secret-safe snapshot', () => {
    ...
    expect((first.snapshot.endpoint.headers as Record<string, unknown>).Authorization).toBe('[REDACTED]')
    expect((first.snapshot.endpoint.headers as Record<string, unknown>)['X-Api-Token']).toBe('[REDACTED]')
