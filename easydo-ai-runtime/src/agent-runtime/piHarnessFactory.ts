import { createModels, createProvider, type Model, type MutableModels, type Api, type Provider, type ProviderHeaders, type OpenAICompletionsCompat } from '@earendil-works/pi-ai'
import { openAICompletionsApi } from '@earendil-works/pi-ai/api/openai-completions.lazy'
import { openAIResponsesApi } from '@earendil-works/pi-ai/api/openai-responses.lazy'
import { openaiProvider } from '@earendil-works/pi-ai/providers/openai'
import { openrouterProvider } from '@earendil-works/pi-ai/providers/openrouter'
import { anthropicProvider } from '@earendil-works/pi-ai/providers/anthropic'
import { AgentHarness, InMemorySessionRepo, formatSkillsForSystemPrompt, type AgentMessage, type AgentTool, type ExecutionEnv, type Skill, type ThinkingLevel } from '@earendil-works/pi-agent-core/node'
...
export interface PiHarnessFactoryOptions {
  defaultExecutionEnv?: ExecutionEnv
  systemPrompt?: string
  tools?: AgentTool[]
  skills?: Skill[]
  historyMaxMessages?: number
  historyMaxChars?: number
  configureModels?: (models: MutableModels, selection: PiModelSelection) => void | Promise<void>
}
...
export function createPiHarnessFactory(options: PiHarnessFactoryOptions) {
  const sessionRepo = new InMemorySessionRepo()
  return async (input: AgentHarnessRunInput): Promise<PiHarnessLike> => {
    ...
    return new AgentHarness({
      ...
      tools: [
        ...filterWorkspaceToolsByPolicy(
          createWorkspaceTools(executionEnv, input.runMode ?? 'write', {
            toolPolicy: input.toolPolicy,
            actorRole: input.actorRole,
            runMode: input.runMode ?? 'write',
            recordEvent: input.recordEvent,
            decideTool: input.decideTool
          }),
          input.toolPolicy
        ),
...
function compactPiHistory(
  ...
  return {
    ...
    compacted: plan.compacted.map((message) => ({
      role: (message.role || 'user') as NonNullable<AgentHarnessRunInput['history']>[number]['role'],
      content: String(message.content || '')
    })),
...
function normalizeModelSelection(selection: PiModelSelection | undefined): PiModelSelection | undefined {
  if (!selection) return undefined
  const inference = asRecord(selection.inference)
  return {
    ...selection,
    thinking_level: selection.thinking_level ?? normalizeThinkingLevel(
...
async function openOrCreateSession(repo: InMemorySessionRepo, sessionID: string) {
  const existing = (await repo.list()).find((metadata) => metadata.id === sessionID)
  if (existing) return { session: await repo.open(existing), created: false }
  return { session: await repo.create({ id: sessionID }), created: true }
}

function piHistoryMessage(
  history: NonNullable<AgentHarnessRunInput['history']>[number],
  selection: PiModelSelection,
  model: Model<Api>
): AgentMessage {
...
export async function configureEasyDoModels(models: MutableModels, selection: PiModelSelection) {
 ...
 */
function ensureSelectionModel(models: MutableModels, selection: PiModelSelection) {
...
function buildModelFromSelection(
  ...
  const api = (selection.api as Api | undefined) ?? baseModel?.api ?? defaultApiForProvider(selection.provider_id)
  ...
  return {
    ...
    contextWindow,
    maxTokens,
    ...(Object.keys(mergedHeaders).length > 0 ? { headers: mergedHeaders as ProviderHeaders } : {}),
    ...(selection.compat ? { compat: selection.compat } : existingModel?.compat ? { compat: existingModel.compat } : {})
  } as Model<Api>
}

function openAIProviderForSelection(selection: PiModelSelection): Provider {
  const provider = openaiProvider()
  if (selection.api !== 'openai-completions') return provider
  ...
  return createProvider({
    ...
    auth: provider.auth,
    models: provider.getModels() as Model<Api>[],
    api: {
...
function defaultApiForProvider(providerID: string): Api {
  switch (providerID) {
    case 'anthropic':
...
export function modelSelectionFromRuntimeConfig(input: {
  ...
  const api = firstString(model.api, provider.api) as Api || defaultApiForRuntimeProvider(providerID, rawProviderID)
  ...
  return {
    ...
    headers: Object.keys({ ...providerHeaders, ...headers }).length > 0 ? { ...providerHeaders, ...headers } as ProviderHeaders : undefined,
...
function runtimeProviderID(provider: Record<string, unknown>, model: Record<string, unknown>) {
  return firstString(
    model.provider_type,
...
function normalizePiProviderID(providerID: string) {
...
function normalizedRuntimeID(value: string) {
  return value.trim().toLowerCase().replace(/[\s_]+/g, '-')
}

function defaultApiForRuntimeProvider(providerID: string, rawProviderID: string): Api | undefined {
...
function runtimeModelID(model: Record<string, unknown>) {
  return firstString(
...
function resolveModel(models: ReturnType<typeof createModels>, providerID: string, modelID: string): Model<any> {
...
function applyProviderOverrides<TApi extends Api>(provider: Provider<TApi>, selection: PiModelSelection): Provider<TApi> {
  if (!selection.base_url && !selection.headers && !selection.api_key) return provider
  return {
    ...provider,
...
function streamOptionsFromSelection(selection: PiModelSelection) {
...
function normalizeThinkingLevel(...values: unknown[]): ThinkingLevel | undefined {
  for (const value of values) {
...
function openAICompatibleCompat(input: {
  ...
  if (input.api !== 'openai-completions') return undefined
  ...
  if (!needsConservativeDefaults && Object.keys(configuredCompat).length === 0) return undefined
  return {
...
function conservativeOpenAICompatibleCompat(): OpenAICompletionsCompat {
  return {
    supportsStore: false,
...
function isCustomOpenAICompatibleRuntimeProvider(providerID: string, rawProviderID: string, baseURL: string) {
  if (providerID !== 'openai') return false
  const normalizedRaw = normalizedRuntimeID(rawProviderID)
  if (normalizedRaw && normalizedRaw !== 'openai') return true
  const normalizedBaseURL = baseURL.trim().toLowerCase()
  return Boolean(normalizedBaseURL && !normalizedBaseURL.includes('api.openai.com'))
}

function configuredOpenAICompat(
  provider: Record<string, unknown>,
  model: Record<string, unknown>
  ...
  return sources.reduce<OpenAICompletionsCompat>((compat, source) => ({
    ...compat,
    ...sanitizeOpenAICompat(asRecord(source))
  }), {})
}

function sanitizeOpenAICompat(source: Record<string, unknown>): OpenAICompletionsCompat {
  ...
  if (isOpenAIThinkingFormat(thinkingFormat)) compat.thinkingFormat = thinkingFormat
  const chatTemplateKwargs = asRecord(source.chatTemplateKwargs || source.chat_template_kwargs)
  if (Object.keys(chatTemplateKwargs).length > 0) compat.chatTemplateKwargs = chatTemplateKwargs as OpenAICompletionsCompat['chatTemplateKwargs']
  const openRouterRouting = asRecord(source.openRouterRouting || source.openrouter_routing || source.open_router_routing)
  if (Object.keys(openRouterRouting).length > 0) compat.openRouterRouting = openRouterRouting
  const vercelGatewayRouting = asRecord(source.vercelGatewayRouting || source.vercel_gateway_routing)
  if (Object.keys(vercelGatewayRouting).length > 0) compat.vercelGatewayRouting = vercelGatewayRouting
  const cacheControlFormat = firstString(source.cacheControlFormat, source.cache_control_format)
  if (cacheControlFormat === 'anthropic') compat.cacheControlFormat = 'anthropic'
  return compat
}

function copyBooleanCompat<T extends keyof OpenAICompletionsCompat>(
  target: OpenAICompletionsCompat,
...
function jsonRecord(...values: unknown[]) {
  for (const value of values) {
...
function isOpenAIThinkingFormat(value: string): value is NonNullable<OpenAICompletionsCompat['thinkingFormat']> {
  return [
    'openai',
    'openrouter',
    'deepseek',
