...
describe('createPiHarnessFactory', () => {
  it('uses the workspace execution environment supplied for the current run', async () => {
    const workspaceEnv = { cwd: '/workspace' } as ExecutionEnv
    ...
    expect((harness as unknown as { env: ExecutionEnv }).env).toBe(workspaceEnv)
    expect((harness as unknown as { getTools: () => Array<{ name: string }> }).getTools().map((tool) => tool.name))
    ...
    const factory = createPiHarnessFactory({
      ...
      configureModels(models, selection) {
        ...
        models.setProvider({
          ...
          getModels: () => [model as never],
          stream: (() => { throw new Error('not used') }) as never,
          streamSimple: (() => { throw new Error('not used') }) as never
        })
      }
    })
  ...
  it('injects available skills into the Pi system prompt', async () => {
    ...
    const turnState = await (harness as unknown as { createTurnState: () => Promise<{ systemPrompt: string }> }).createTurnState()
  ...
  it('versions and compacts canonical history before rebuilding the Pi model context', async () => {
    ...
    const history = [
      { role: 'user' as const, content: 'old user message '.repeat(8), timestamp: 1, status: 'completed' },
      { role: 'assistant' as const, content: 'old assistant response '.repeat(8), timestamp: 2, status: 'completed' },
      { role: 'user' as const, content: 'recent user', timestamp: 3, status: 'completed' },
      { role: 'assistant' as const, content: 'recent assistant', timestamp: 4, status: 'completed' }
    ]
    ...
    const makeFactory = () => createPiHarnessFactory({
      ...
      configureModels(models, selection) {
        models.setProvider({
          ...
          name: selection.provider_id,
          auth: { apiKey: { name: 'test', resolve: async () => ({ auth: { apiKey: 'test' } }) } },
          getModels: () => [{ ...model, provider: selection.provider_id, id: selection.id } as never],
          stream: (() => { throw new Error('not used') }) as never,
          streamSimple: (() => { throw new Error('not used') }) as never
        })
      }
  ...
  it('dispatches OpenAI-compatible custom models through Pi chat-completions payloads', async () => {
    ...
    await configureEasyDoModels(models, selection)
    const model = models.getModel('openai', 'sensenova-6.7-flash-lite')
    const provider = models.getProvider('openai') as {
      streamSimple: (
        model: unknown,
        context: unknown,
        options: { apiKey: string; onPayload(payload: unknown): unknown }
...
function harnessSession(harness: unknown) {
  const session = (harness as {
    session?: {
      appendCustomEntry(customType: string, data?: unknown): Promise<string>
      getEntries(): Promise<Array<Record<string, unknown>>>
      buildContext(): Promise<{ messages: Array<Record<string, unknown>> }>
    }
  }).session
  if (!session) throw new Error('Pi harness session is unavailable')
  return session
}

function harnessStreamOptions(harness: unknown) {
  const getStreamOptions = (harness as {
    getStreamOptions?: () => { headers?: Record<string, string>; maxRetries?: number; timeoutMs?: number }
  }).getStreamOptions
  if (!getStreamOptions) throw new Error('Pi harness stream options are unavailable')
  return getStreamOptions.call(harness)
}

function harnessThinkingLevel(harness: unknown) {
  const getThinkingLevel = (harness as {
    getThinkingLevel?: () => string
  }).getThinkingLevel
  if (!getThinkingLevel) throw new Error('Pi harness thinking level is unavailable')
