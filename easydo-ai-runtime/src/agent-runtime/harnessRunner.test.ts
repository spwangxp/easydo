...
class UsageHarness implements PiHarnessLike {
  subscribe() {
    return () => {}
  }
...
class PiCamelUsageHarness implements PiHarnessLike {
  subscribe() {
    return () => {}
  }
...
class PiTotalOnlyUsageHarness implements PiHarnessLike {
  subscribe() {
    return () => {}
  }
...
class SkillInvocationHarness implements PiHarnessLike {
  promptCalled = false
  skillCall: { name: string; instructions?: string } | null = null
  private listeners: Array<(event: unknown) => void | Promise<void>> = []
  ...
  async skill(name: string, instructions?: string) {
...
class ApprovalHarness implements PiHarnessLike {
  subscribe() {
    return () => {}
  }
...
class EventApprovalHarness implements PiHarnessLike {
...
class FailureMessageHarness implements PiHarnessLike {
  subscribe() {
    return () => {}
  }
...
class CompactionHarness implements PiHarnessLike {
  private listeners: Array<(event: unknown) => void | Promise<void>> = []
...
class AbortableHarness implements PiHarnessLike {
  aborted = false
  private listeners: Array<(event: unknown) => void | Promise<void>> = []
...
class SafeCheckpointHarness implements PiHarnessLike {
  private listeners: Array<(event: unknown) => void | Promise<void>> = []
...
class SteerableHarness implements PiHarnessLike {
...
describe('AgentHarnessRunner', () => {
  ...
  it('calls the safe checkpoint callback for Pi save point event shapes', async () => {
    const store = new InMemoryAgentEventStore()
    const checkpoints: Array<{ runtime_run_id: string, checkpoint: 'turn_save_point' }> = []
  ...
  it('remembers an exact run abort while its harness is still being created', async () => {
    const store = new InMemoryAgentEventStore()
    const harness = new AbortableHarness()
    let releaseHarness: ((harness: PiHarnessLike) => void) | undefined
    const harnessGate = new Promise<PiHarnessLike>((resolve) => { releaseHarness = resolve })
...
class ReasoningHarness implements PiHarnessLike {
  private listeners: Array<(event: unknown) => void | Promise<void>> = []
  ...
  async prompt(text: string) {
    const partial = (content: unknown[]) => ({ role: 'assistant', content })
    for (const listener of this.listeners) {
      await listener({
        type: 'message_update',
...
describe('AgentHarnessRunner reasoning bridge', () => {
  ...
  it('skips session.text.ended when assistant produces only reasoning and no text', async () => {
    ...
    class ReasoningOnlyHarness implements PiHarnessLike {
      private listeners: Array<(event: unknown) => void | Promise<void>> = []
      subscribe(listener: (event: unknown) => void | Promise<void>) {
        this.listeners.push(listener)
