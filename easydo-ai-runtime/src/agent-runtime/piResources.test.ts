...
import type { AgentProfileSnapshot, AgentResource } from '../domain/runtime.js'
...
describe('buildPiHarnessResources', () => {
  ...
  it('keeps the complete MCP and subagent catalog in write mode', () => {
    ...
    expect(built.tools.map((tool) => tool.name)).toEqual(['read_tool', 'write_tool', 'unknown_tool', 'easydo_subagent_spawn'])
  })
  ...
  it('maps selected skill, MCP, and subagent resources into Pi harness resources and tools', () => {
    ...
    expect(built.tools.map((tool) => tool.name)).toEqual(['easydo_resource_list', 'easydo_subagent_spawn', 'easydo_skill_load'])
  ...
  it('executes MCP tools through the supplied adapter and records projectable OpenCode-style tool events', async () => {
    const events: Array<{ type: string; assistant_message_id?: string; call_id?: string; tool?: string; tool_name?: string; content?: unknown; structured?: unknown; error?: { message: string } }> = []
    const calls: Array<{ callID: string; toolName: string; args: Record<string, unknown> }> = []
    const built = buildPiHarnessResources({
      profile: profile({
        mcp_servers: [{
  ...
  it('applies server-scoped MCP rules before Pi tool execution', async () => {
    const events: Array<{ type: string; decision?: string; matched_rule?: string; permission_key?: string }> = []
    let executions = 0
    const built = buildPiHarnessResources({
      profile: profile({
        mcp_servers: [{
  ...
  it('asks by default for MCP tools that are not configured on the profile reference', async () => {
    const events: Array<{ type: string; decision?: string; matched_rule?: string }> = []
    let executed = false
    const built = buildPiHarnessResources({
      profile: profile({ mcp_servers: [{ resource_type: 'mcp_server', resource_id: 'mcp-1' }] }),
      resources: [resource({
  ...
  it('asks and resolves permission before executing confirmation-gated MCP tools with projectable approval fields', async () => {
    const events: Array<{ type: string; result?: string; request_id?: string; approval_id?: string; message?: string }> = []
    let executed = false
    const built = buildPiHarnessResources({
      profile: profile({ mcp_servers: [{ resource_type: 'mcp_server', resource_id: 'mcp-1' }] }),
      resources: [resource({
  ...
  it('requires permission for MCP tools when the profile policy requires all tools', async () => {
    ...
    const built = buildPiHarnessResources({
      ...
      recordEvent: async (event) => { events.push(event as never) }
    })
  ...
  it('allows explicitly configured read MCP tools without guessing write intent from the tool name', async () => {
    ...
    const built = buildPiHarnessResources({
      ...
      recordEvent: async (event) => { events.push(event as { type: string }) }
    })
  ...
  it('executes subagent.spawn through the supplied child-run adapter and records projectable child link output', async () => {
    const events: Array<{ type: string; tool?: string; call_id?: string; content?: unknown; structured?: unknown }> = []
    const calls: Array<{ callID: string; subagentID: number; task: string }> = []
    const built = buildPiHarnessResources({
    ...
    const result = await built.tools.find((tool) => tool.name === 'easydo_subagent_spawn')?.execute('call-subagent-1', {
      subagent_id: 9,
