...
export class RuntimeMetrics {
  ...
  private readonly observations = new Map<string, Array<{ value: number, observedAt: number }>>()
  ...
  increment(name: string, labels: MetricLabels = {}, value = 1) {
    ...
    else this.counters.set(normalized.key, { name, labels: normalized.labels, value })
  }
  ...
  addGauge(name: string, delta: number, labels: MetricLabels = {}) {
    const normalized = this.normalize(name, labels)
  ...
  observe(name: string, value: number, labels: MetricLabels = {}, observedAt = Date.now()) {
    const normalized = this.normalize(name, labels)
  ...
  latencySummary(name: string, windowMs: number, observedAt = Date.now()): RuntimeLatencySummary {
  ...
  private normalize(name: string, labels: MetricLabels) {
    ...
    for (const label of Object.keys(combined)) {
      if (!ALLOWED_LABELS.has(label)) throw new Error(`Metric label ${label} is not allowed`)
    }
