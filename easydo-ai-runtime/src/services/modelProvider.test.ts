...
import type { AgentProfileSnapshot } from '../domain/runtime.js'
...
function openAIProfile(): AgentProfileSnapshot {
  return {
    id: 1,
    workspace_id: 11,
  ...
  } as unknown as AgentProfileSnapshot
}

describe('ProviderAdapterChatModelClient', () => {
  it('logs correlated provider outcomes without logging prompts or credentials', async () => {
    const lines: string[] = []
    const client = new ProviderAdapterChatModelClient(
      async () => new Response(JSON.stringify({ choices: [{ message: { role: 'assistant', content: 'ok' } }] }), {
        status: 200,
        headers: { 'content-type': 'application/json' }
      }),
      undefined,
      createRuntimeLogger({ sink: (line) => lines.push(line) })
    )

    await client.complete({
      profile: openAIProfile(),
      session: { id: 9, workspace_id: 11 } as never,
      history: [],
  ...
  it('preserves leading whitespace in streamed reasoning and answer deltas', async () => {
    ...
    const client = new ProviderAdapterChatModelClient(async () => new Response(
    ...
    const deltaEvents = events.filter(
      (event): event is Extract<typeof event, { type: 'reasoning_delta' | 'answer_delta' }> =>
        event.type === 'reasoning_delta' || event.type === 'answer_delta'
    )
    expect(deltaEvents.map((event) => event.delta).join('')).toBe(
      'The user says: "add a pipeline".\nI should ask questions.先确认： **目标** 和触发方式。'
    )
