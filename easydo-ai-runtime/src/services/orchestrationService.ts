...
export type OrchestrationEventSink = (type: string, payload: Record<string, unknown>) => Promise<unknown> | unknown
...
export type DispatchTaskExecutor = (task: OrchestrationTask, plan: OrchestrationPlan) => Promise<{
  status: 'completed' | 'failed' | 'cancelled' | 'blocked_approval' | 'running'
...
export class OrchestrationService {
  ...
  async assembleAndRun(
    ...
    options: {
      executeTask: DispatchTaskExecutor
  ...
  async assembleContext(input: AssembleOrchestrationInput, emit?: OrchestrationEventSink) {
    ...
    const contextPack: OrchestrationContextPack = {
  ...
  async drive(
    ...
    if (!plan) throw new Error(`orchestration plan not found: ${orchestrationID}`)
  ...
  private async runDispatchCollectLoop(
    ...
    for (let wave = 0; wave < 16; wave += 1) {
      ...
      if (pendingAfter === 0 || (pendingAfter === pendingBefore && !running && wave > 0)) break
      if (pendingAfter > 0 && pendingAfter < pendingBefore) continue
      if (pendingAfter > 0 && wave === 0) continue
  ...
  async dispatchReadyTasks(
    ...
    const ready = tasks.filter((task) => {
      ...
      if (task.dependency_ids.length === 0) return true
    ...
    await Promise.all(ready.map(async (task) => {
      ...
      try {
        ...
        const nextStatus: OrchestrationTaskStatus =
  ...
  async collectStatus(plan: OrchestrationPlan, emit?: OrchestrationEventSink) {
    await this.transitionPhase(plan.orchestration_id, 'collect_status', emit)
    const tasks = await this.store.listOrchestrationTasks(plan.workspace_id, plan.orchestration_id)
    for (const task of tasks) {
      if (!task.child_runtime_run_id || TERMINAL_TASK_STATUSES.has(task.status)) continue
      const childRun = await this.store.getRun(plan.workspace_id, task.child_runtime_run_id)
      if (!childRun) continue
      if (!isTerminalRunStatus(childRun.status)) continue
      const nextStatus: OrchestrationTaskStatus =
        childRun.status === 'completed' ? 'completed'
  ...
  async synthesizeResults(plan: OrchestrationPlan, emit?: OrchestrationEventSink) {
  ...
  async reviewAndMaybeCreateFollowups(plan: OrchestrationPlan, emit?: OrchestrationEventSink) {
    ...
    const tasks = await this.store.listOrchestrationTasks(plan.workspace_id, plan.orchestration_id)
    const failedWorkers = tasks.filter((task) =>
      (task.task_kind === 'worker') && (task.status === 'failed' || task.status === 'cancelled')
    )
    const existingFollowups = tasks.filter((task) => task.task_kind === 'followup')
    if (failedWorkers.length === 0 || existingFollowups.length > 0) return []

    const created: OrchestrationTask[] = []
    let nextSeq = Math.max(0, ...tasks.map((task) => task.task_seq)) + 1
    for (const failed of failedWorkers) {
      const followup: OrchestrationTask = {
  ...
  async finalizeSummary(plan: OrchestrationPlan, emit?: OrchestrationEventSink) {
  ...
  private async transitionPhase(orchestrationID: string, phase: OrchestrationPhase, emit?: OrchestrationEventSink) {
    const plan = await this.findPlanByID(orchestrationID)
    if (!plan) return
    if (plan.phase === phase || plan.phase === 'completed') return
  ...
  private async requirePlan(workspaceID: number, orchestrationID: string) {
  ...
  private async findPlanByID(orchestrationID: string) {
    return this.store.getOrchestrationPlan(0, orchestrationID)
  }
}

function taskEventPayload(task: OrchestrationTask, plan: OrchestrationPlan) {
  return {
    orchestration_id: plan.orchestration_id,
    parent_runtime_run_id: plan.parent_runtime_run_id,
    task_id: task.task_id,
...
function allWorkerTasksTerminal(tasks: OrchestrationTask[]) {
...
function orchestrationBusinessID(workspaceID: number, parentRuntimeRunID: string) {
...
function leaseExpiry(nowISO: string) {
