...
import type { AgentResource } from '../domain/runtime.js'
...
function repositoryConfig(resource: AgentResource) {
...
function isLocalRepositoryURL(url: string) {
  return url.startsWith('file://') || isAbsolute(url)
}

async function resolveRemoteCommit(url: string, branch: string, commit: string) {
  const explicit = commit.trim()
  if (/^[a-f0-9]{40,64}$/i.test(explicit)) return explicit.toLowerCase()
  ...
  try {
    ...
    if (/^[a-f0-9]{40,64}$/i.test(digest)) return digest.toLowerCase()
  } catch (error) {
  ...
  throw new Error(
    `Skill repository commit is required and must be a full Git commit digest (branch "${ref}" could not be resolved)`
  )
}
...
async function repositoryRoot(url: string, branch: string, commit: string) {
...
async function findSkillFiles(rootDir: string, depth = 0, found: string[] = []) {
  if (found.length >= MAX_SKILL_FILES || depth > MAX_SCAN_DEPTH) return found
...
function readableKeySegment(value: unknown, fallback = 'skill', maxLength = 64) {
...
function skillNameFallback(relativePath: string) {
  const normalized = relativePath.replaceAll('\\', '/')
  if (normalized.toLowerCase() === 'skill.md') return 'skill'
  const directoryName = basename(dirname(normalized))
  return directoryName && directoryName !== '.' ? directoryName : 'skill'
}

function skillKey(repositoryKey: string, skillName: string, skillPath: string) {
  const suffix = readableKeySegment(skillName || skillPath)
  return `${repositoryKey}/${suffix}`
}

async function parseSkillFile(file: string, scanRoot: string, repository: AgentResource, commit: string): Promise<DiscoveredSkill> {
...
export async function scanSkillRepository(resource: AgentResource) {
...
export async function readSkillRepositoryEntry(repository: AgentResource, entry: string) {
