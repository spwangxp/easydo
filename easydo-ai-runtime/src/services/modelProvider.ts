...
export type ModelOutputPart =
  ...
  | { type: 'reasoning', text: string, raw?: Record<string, unknown> }
  | { type: 'tool_call', tool_call: ChatModelToolCall, raw?: Record<string, unknown> }
  | { type: 'refusal', text: string, raw?: Record<string, unknown> }
  | { type: 'warning', text: string, raw?: Record<string, unknown> }
...
type EndpointDialect = 'openai-chat' | 'openai-responses' | 'anthropic-messages' | 'gemini-generate-content'
...
export interface ResolvedModelRuntime {
  ...
  stream_parser: string
  tool_protocol: string
  tool_result_formatter: string
  reasoning_policy: string
  json_schema_policy: string
  capabilities_internal: Record<string, unknown>
  diagnostics: Record<string, unknown>
}

export type ChatModelStreamEvent =
  | { type: 'reasoning_delta' | 'answer_delta', delta: string, raw?: Record<string, unknown> }
  | { type: 'tool_call', tool_call: ChatModelToolCall, raw?: Record<string, unknown> }

export interface ChatModelClient {
  complete(request: ChatModelRequest): Promise<ChatModelResult | null>
...
function deltaString(...values: unknown[]) {
  for (const value of values) {
...
function seq36(value: number, width = 4) {
...
function readableIDSegment(value: unknown, fallback = 'tool', maxLength = 64) {
...
function providerLabel(profile: AgentProfileSnapshot) {
  return firstString(
    profile.provider.provider_type,
    profile.provider.type,
    profile.provider.name,
    profile.provider.id,
    profile.binding.provider_type,
    'model provider'
  )
}

function providerType(profile: AgentProfileSnapshot) {
  return firstString(
    profile.provider.provider_type,
    profile.provider.type,
    profile.provider.id,
...
function normalizedProviderType(profile: AgentProfileSnapshot) {
  return providerType(profile).replace(/_/g, '-')
}

function isOpenRouterProfile(profile: AgentProfileSnapshot) {
  return normalizedProviderType(profile) === 'openrouter' || resolveBaseURL(profile).includes('openrouter.ai')
}

function providerRuntimeSettings(profile: AgentProfileSnapshot) {
  return {
...
function resolveBaseURL(profile: AgentProfileSnapshot) {
  const baseURL = firstString(
    profile.provider.base_url,
    profile.provider.endpoint,
    profile.binding.base_url,
...
function resolveEndpointOverride(profile: AgentProfileSnapshot, key: string) {
...
function resolveModel(profile: AgentProfileSnapshot) {
  const model = firstString(
    profile.model.provider_model_key,
    profile.model.model,
    profile.model.model_id,
    profile.model.name,
    profile.binding.provider_model_key
  )
  if (!model) {
...
function credentialAuthHeader(profile: AgentProfileSnapshot) {
  const ref = asRecord(profile.provider_credential_ref)
  const secretRef = asRecord(ref.secret_ref)
  const credential = resolveCredential(profile)
  const headerName = firstString(
    ref.auth_header,
    ref.header_name,
    ref.api_key_header,
    secretRef.auth_header,
    secretRef.header_name,
    secretRef.api_key_header,
    'Authorization'
  )
  if (!/^authorization$/i.test(headerName)) {
    return { headerName, headerValue: credential }
  }
  const tokenType = firstString(
    ref.token_type,
    ref.auth_scheme,
    secretRef.token_type,
    secretRef.auth_scheme,
    'Bearer'
  )
...
function providerConfiguredHeaders(profile: AgentProfileSnapshot) {
  const rawHeaders = {
...
function jsonProviderHeaders(profile: AgentProfileSnapshot, authHeader: { headerName: string, headerValue: string }, defaults: Record<string, string> = {}) {
  return {
...
function resolveEndpointDialect(profile: AgentProfileSnapshot): EndpointDialect | undefined {
  const type = normalizedProviderType(profile)
  const endpointOverride = resolveEndpointOverride(profile, 'llm_endpoint').toLowerCase()
  if (endpointOverride.includes('/chat/completions') || endpointOverride === 'chat/completions') return 'openai-chat'
  if (endpointOverride.includes('/responses') || endpointOverride === 'responses') return 'openai-responses'
  if (type === 'anthropic' || type === 'claude') return 'anthropic-messages'
  if (type === 'google' || type === 'gemini' || type === 'google-gemini') return 'gemini-generate-content'
  if (['openai', 'openai-responses', 'responses', 'openai-response'].includes(type)) return 'openai-responses'
...
function providerFamilyForDialect(dialect: EndpointDialect) {
  if (dialect === 'openai-chat') return 'openai-compatible'
  if (dialect === 'openai-responses') return 'openai'
  if (dialect === 'anthropic-messages') return 'anthropic'
  return 'google-gemini'
}

function endpointForDialect(dialect: EndpointDialect, baseURL: string, model: string, endpointOverride = '') {
  if (endpointOverride) {
    const parsed = URL.canParse(endpointOverride) ? new URL(endpointOverride) : null
    if (parsed?.protocol && parsed.host) return endpointOverride
    return joinURL(baseURL, endpointOverride)
  }
  if (dialect === 'openai-chat') return joinURL(baseURL, '/chat/completions')
  if (dialect === 'openai-responses') return joinURL(baseURL, '/responses')
  if (dialect === 'anthropic-messages') return joinURL(baseURL, '/messages')
  return joinURL(baseURL, `/models/${encodeURIComponent(model)}:generateContent`)
}

export class ProviderAdapterRegistry {
  resolve(profile: AgentProfileSnapshot): ResolvedModelRuntime | null {
...
function providerErrorFromResponse(profile: AgentProfileSnapshot, status: number, payload: Record<string, unknown>) {
  ...
  if (authFailure) {
    ...
    return new ModelProviderError(
      ...
      `模型服务认证失败（${target}）：${message}。请检查 AI Provider 绑定的 token/API Key 是否有效。`,
      status
    )
  }
...
function isNativeToolUnsupportedMessage(message: string) {
...
function resolvePromptString(prompt: Record<string, unknown>, key: string) {
...
function hasEnforcedOutputSchema(value: unknown) {
...
function reasoningOptions(profile: AgentProfileSnapshot) {
  if (!isOpenRouterProfile(profile)) return {}
  const inference = asRecord(profile.inference)
  const configuredReasoning = asRecord(inference.reasoning)
  const rawLevel = firstString(
...
function entryPayloadSummary(entry: AISessionEntry) {
  const payload = {
...
function appendModelVisibleHistoryEntry(
...
function buildMessages(request: ChatModelRequest, extraSystemMessages: string[] = []) {
...
function extractDeltaText(delta: Record<string, unknown>) {
  ...
  if (typeof content === 'string') return deltaString(content)
  if (Array.isArray(content)) {
    return content
      .map((block) => {
        const record = asRecord(block)
...
function parseToolArguments(value: unknown) {
  ...
  if (isRecordLike(value)) return asRecord(value)
...
function normalizeToolCall(rawValue: unknown, index = 0): ChatModelToolCall | null {
...
function extractToolCallsFromMessage(message: Record<string, unknown>) {
  const rawToolCalls = Array.isArray(message.tool_calls) ? message.tool_calls : []
  return rawToolCalls
    .map((item, index) => normalizeToolCall(item, index))
    .filter(Boolean) as ChatModelToolCall[]
}

function textFromParts(parts: ModelOutputPart[]) {
  return parts
    .filter((part): part is Extract<ModelOutputPart, { type: 'text' | 'refusal' | 'warning' }> =>
      part.type === 'text' || part.type === 'refusal' || part.type === 'warning')
    .map((part) => part.text.trim())
    .filter(Boolean)
    .join('\n')
}

function toolCallsFromParts(parts: ModelOutputPart[]) {
  return parts
    .filter((part): part is Extract<ModelOutputPart, { type: 'tool_call' }> => part.type === 'tool_call')
    .map((part) => part.tool_call)
}

function hasUsablePart(part: ModelOutputPart) {
  if (part.type === 'tool_call') return true
  if (part.type === 'reasoning') return false
  return Boolean(part.text.trim())
}

function finishReasonFromOpenAIChat(payload: Record<string, unknown>) {
  const choices = Array.isArray(payload.choices) ? payload.choices : []
...
function resultFromParts(params: {
  ...
  empty_details?: Record<string, unknown>
}): ChatModelResult {
...
function providerResponseDiagnostics(runtime: ResolvedModelRuntime, payload: Record<string, unknown>, rawText: string, attempt: number) {
  const choices = Array.isArray(payload.choices) ? payload.choices : []
  const firstChoice = asRecord(choices[0])
  const message = asRecord(firstChoice.message)
  const content = message.content
...
function isProviderEmptyResponse(error: unknown) {
  return error instanceof ModelProviderError && error.code === 'provider_response_empty'
}

function isNativeToolUnsupportedError(error: unknown) {
...
function parseOpenAIChatParts(payload: Record<string, unknown>) {
  const choices = Array.isArray(payload.choices) ? payload.choices : []
  const firstChoice = asRecord(choices[0])
  const message = asRecord(firstChoice.message)
  const parts: ModelOutputPart[] = []
  const reasoning = firstString(message.reasoning, message.reasoning_content, message.thinking)
  if (reasoning) parts.push({ type: 'reasoning', text: reasoning, raw: message })
  const text = extractText(payload)
  if (text) parts.push({ type: 'text', text, raw: message })
...
function parseOpenAIResponsesParts(payload: Record<string, unknown>) {
  const parts: ModelOutputPart[] = []
  ...
  for (const item of output) {
    ...
    if (type === 'reasoning') {
      ...
      const text = summary
        ...
        .filter(Boolean)
        .join('\n')
      if (text) parts.push({ type: 'reasoning', text, raw: record })
      continue
    }
    if (type === 'function_call') {
      ...
      toolCallIndex += 1
      if (toolCall) parts.push({ type: 'tool_call', tool_call: toolCall, raw: record })
      continue
    }
    if (type === 'message') {
...
function parseAnthropicParts(payload: Record<string, unknown>) {
  const parts: ModelOutputPart[] = []
  ...
  for (const item of content) {
    ...
    if (type === 'text') {
      ...
      if (text) parts.push({ type: 'text', text, raw: record })
      continue
    }
    if (type === 'thinking') {
...
function parseGeminiParts(payload: Record<string, unknown>) {
  ...
  for (const item of rawParts) {
    ...
    if (Object.keys(functionCall).length > 0) {
      ...
      toolCallIndex += 1
      if (toolCall) parts.push({ type: 'tool_call', tool_call: toolCall, raw: functionCall })
    }
  }
  return parts
}

function responseFormatOptions(request: ChatModelRequest) {
  if (hasEnforcedOutputSchema(request.output_schema)) {
    return {
      response_format: {
        type: 'json_schema',
...
function textToolProtocolInstruction(request: ChatModelRequest) {
...
function toolOptions(request: ChatModelRequest) {
  const tools = Array.isArray(request.tools) ? request.tools : []
  const definitions = tools
    .map((tool) => ({
      name: firstString(tool.name),
...
function openAIResponsesToolOptions(request: ChatModelRequest) {
  const tools = Array.isArray(request.tools) ? request.tools : []
  const definitions = tools
    .map((tool) => ({
      type: 'function',
...
function anthropicToolOptions(request: ChatModelRequest) {
  const tools = Array.isArray(request.tools) ? request.tools : []
  const definitions = tools
    .map((tool) => ({
      name: firstString(tool.name),
...
function geminiToolOptions(request: ChatModelRequest) {
  const tools = Array.isArray(request.tools) ? request.tools : []
...
function buildOpenAIResponsesInput(request: ChatModelRequest) {
...
function buildAnthropicMessages(request: ChatModelRequest) {
  const system: string[] = []
...
function buildGeminiContents(request: ChatModelRequest) {
  return buildMessages(request).map((message) => ({
...
async function* parseOpenAICompatibleStream(response: Response): AsyncIterable<ChatModelStreamEvent> {
  if (!response.body) {
  ...
  const mergeToolCallDelta = (rawValue: unknown, position: number) => {
    const raw = asRecord(rawValue)
  ...
  function* flushToolCalls(raw: Record<string, unknown>): Iterable<ChatModelStreamEvent> {
  ...
  function* handleDataLine(data: string): Iterable<ChatModelStreamEvent> {
    if (!data) return
    if (data === '[DONE]') {
...
export class ProviderAdapterChatModelClient implements ChatModelClient {
  constructor(
  ...
  async complete(request: ChatModelRequest): Promise<ChatModelResult | null> {
    ...
    try {
      let result: ChatModelResult | null = null
      if (runtime.endpoint_dialect === 'openai-chat') result = await this.completeOpenAIChat(request, runtime)
      if (runtime.endpoint_dialect === 'openai-responses') result = await this.completeOpenAIResponses(request, runtime)
      if (runtime.endpoint_dialect === 'anthropic-messages') result = await this.completeAnthropicMessages(request, runtime)
      if (runtime.endpoint_dialect === 'gemini-generate-content') result = await this.completeGeminiGenerateContent(request, runtime)
  ...
  private async retryProviderEmptyResponse<T>(operation: (attempt: number) => Promise<T>): Promise<T> {
    try {
      return await operation(1)
    } catch (error) {
      if (!isProviderEmptyResponse(error)) throw error
    }
    return operation(2)
  }

  private async recoverNativeToolUnsupported<T>(
    request: ChatModelRequest,
    nativeOperation: (attempt: number) => Promise<T>,
    textToolFallbackOperation: (attempt: number) => Promise<T>
  ): Promise<T> {
    try {
      return await this.retryProviderEmptyResponse(nativeOperation)
    } catch (error) {
      if (!isNativeToolUnsupportedError(error) || !textToolProtocolInstruction(request)) throw error
    }
    return this.retryProviderEmptyResponse(textToolFallbackOperation)
  }

  private async completeOpenAIChat(request: ChatModelRequest, runtime: ResolvedModelRuntime): Promise<ChatModelResult> {
    const authHeader = credentialAuthHeader(request.profile)
    const inference = asRecord(request.profile.inference)
    const buildBody = (nativeTools: boolean): Record<string, unknown> => ({
      model: runtime.model,
    ...
    const completeWithBody = async (attempt: number, nativeTools: boolean) => {
    ...
    return this.recoverNativeToolUnsupported(
      request,
      (attempt) => completeWithBody(attempt, true),
      (attempt) => completeWithBody(attempt, false)
    )
  }

  private async completeOpenAIResponses(request: ChatModelRequest, runtime: ResolvedModelRuntime): Promise<ChatModelResult> {
    const authHeader = credentialAuthHeader(request.profile)
    const inference = asRecord(request.profile.inference)
  ...
  private async completeAnthropicMessages(request: ChatModelRequest, runtime: ResolvedModelRuntime): Promise<ChatModelResult> {
  ...
  private async completeGeminiGenerateContent(request: ChatModelRequest, runtime: ResolvedModelRuntime): Promise<ChatModelResult> {
    const credential = resolveCredential(request.profile)
    const inference = asRecord(request.profile.inference)
    const body: Record<string, unknown> = {
    ...
    try {
      if (runtime.endpoint_dialect !== 'openai-chat') {
        ...
        for (const part of completion?.parts || []) {
          if (part.type === 'reasoning') yield { type: 'reasoning_delta', delta: part.text, raw: part.raw }
          if (part.type === 'text') yield { type: 'answer_delta', delta: part.text, raw: part.raw }
          if (part.type === 'tool_call') yield { type: 'tool_call', tool_call: part.tool_call, raw: part.raw }
        }
      ...
      const fetchStream = async (nativeTools: boolean) => {
        const response = await this.fetchImpl(runtime.endpoint, {
          method: 'POST',
          headers: jsonProviderHeaders(request.profile, authHeader, { 'X-Title': 'EasyDo Page Assistant' }),
          body: JSON.stringify(buildBody(nativeTools))
...
function providerLogContext(request: ChatModelRequest, runtime: ResolvedModelRuntime, operation: string): RuntimeLogInput {
  return {
...
export class FallbackChatModelClient implements ChatModelClient {
  async complete(request: ChatModelRequest): Promise<ChatModelResult> {
...
export class CompositeChatModelClient implements ChatModelClient {
  constructor(private readonly clients: ChatModelClient[]) {}
